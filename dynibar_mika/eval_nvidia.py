...
CUDA_VISIBLE_DEVICES=0,1 python eval_nvidia.py --config configs_nvidia/eval_ballon1_long.txt
CUDA_VISIBLE_DEVICES=2,3 python eval_nvidia.py --config configs_nvidia/eval_ballon2_long.txt
...
CUDA_VISIBLE_DEVICES=0,1 python eval_nvidia.py --config configs_nvidia/eval_truck_long.txt
CUDA_VISIBLE_DEVICES=2,3 python eval_nvidia.py --config configs_nvidia/eval_umbrella_long.txt

CUDA_VISIBLE_DEVICES=0,1 python eval_nvidia.py --config configs_nvidia/eval_dynamicFace_long.txt
CUDA_VISIBLE_DEVICES=2,3 python eval_nvidia.py --config configs_nvidia/eval_playground_long.txt
    ...
    def __getitem__(self, idx): # idx corresponds to 
        ...
        static_pose_ids = get_nearest_pose_ids(render_pose,
                                            ...
                                            #    tar_id=self.render_idx, # do not include target image itself
                                               tar_id=idx, # do not include target image itself
...
if __name__ == '__main__':
    ...
    model = DynibarZQ(args, load_scheduler=False, load_opt=False)
    ...
    for img_i in range(3, args.num_frames-3, 2):
        ...
        for i, data in enumerate(test_loader):
            ...
            fine_pred_rgb = ret['outputs_coarse_ref']['rgb'].detach().cpu().numpy()
