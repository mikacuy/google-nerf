...
class MVSSystem(LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            img_err_abs = torch.nan_to_num(img_err_abs)
            # print(mse2psnr(torch.mean(img_err_abs ** 2)))
            log['val_psnr_all'] = mse2psnr(torch.mean(img_err_abs ** 2))

            rgbs = torch.clamp(rgbs, 0, 1)
            rgbs = torch.nan_to_num(rgbs)
            
            ssim = structural_similarity(rgbs.cpu().numpy(), img.cpu().numpy(), data_range=1., channel_axis=-1)
            lpips = lpips_alex(rgbs.permute(2, 0, 1).unsqueeze(0), img.permute(2, 0, 1).unsqueeze(0), normalize=True)[0]

            log['val_ssim_all'] = ssim
            log['val_lpips_all'] = lpips
    ...
    def validation_epoch_end(self, outputs):
        ...
        mean_ssim_all = np.stack([x['val_ssim_all'] for x in outputs]).mean()
