...
from load_blender import load_blender_data, load_scene_blender_fixed_dist_new, load_scene_blender2, load_scene_blender_fixed_dist_video
...
def render_video(poses, H, W, K, filename, args, render_kwargs_test, fps=5):
    ...
    video_file = os.path.join(args.ckpt_dir, args.expname, filename + '_slow.mp4')
    # video_file = os.path.join(args.ckpt_dir, args.expname, filename + '_reverse.mp4')
    ...
    # imageio.mimsave(video_file,
    #                 [imageio.imread(os.path.join(video_dir, img)) for img in imgs],
    #                 fps=8, macro_block_size=1)

    # imageio.mimsave(video_file,
    #                 [imageio.imread(os.path.join(video_dir, img)) for img in imgs[::-1]],
    #                 fps=12, macro_block_size=1)
...
def train():
    ...
    if "chair" in args.scene_id:
        # idx_to_take = [0, 26, 5, 74, 81, 83, 97, 60]
        # idx_to_take = [97]
        idx_to_take = [0]

    ## Mic
    elif "mic" in args.scene_id:
        # idx_to_take = [16, 34, 51, 58, 68, 6, 44, 66]
        idx_to_take = [0]

    ## Lego
    elif "lego" in args.scene_id:
        idx_to_take = [24, 0, 11, 36, 37, 45, 47, 49]

    start_time = time.time()
    for i in idx_to_take:
        print("Processing pose idx: "+str(i))
        imgs, poses, render_poses, hwf, i_split = load_scene_blender_fixed_dist_video(scene_data_dir, half_res=args.half_res, train_dist=1.0, test_dist=1.0, video_idx=i)
        ...
        render_video(vposes, H, W, K, args.scene_id+"_final4_"+str(i), args, render_kwargs_test, fps=10)
