'''
Allow no view positional encoding for the coarse network
This seems to be causing training instability
'''
import os, sys
import numpy as np
import imageio
...
def create_nerf(args):
    ...
    if args.use_viewdirs:
        ...
        ### For coarse network
        embeddirs_fn_coarse, input_ch_views_coarse = get_embedder(args.multires_views_coarse, args.i_embed)
    ...
    model = NeRF(D=args.netdepth, W=args.netwidth,
                 ...
                 input_ch_views=input_ch_views_coarse, use_viewdirs=args.use_viewdirs).to(device)
    ...
    network_query_fn_coarse = lambda inputs, viewdirs, network_fn : run_network(inputs, viewdirs, network_fn,
                                                                embed_fn=embed_fn,
                                                                embeddirs_fn=embeddirs_fn_coarse,
                                                                netchunk=args.netchunk)
    ...
    render_kwargs_train = {
        ...
        'network_query_fn_coarse' : network_query_fn_coarse,
...
#     raw = run_network(pts)
    raw = network_query_fn_coarse(pts, viewdirs, network_fn)
...
def config_parser():
    ...
    parser.add_argument("--multires_views_coarse", type=int, default=0, 
                        help='log2 of max freq for positional encoding (2D direction) for the coarse network')
