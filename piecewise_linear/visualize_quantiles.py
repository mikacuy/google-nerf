'''
May 6, 2023
Render quantiles and visualize samples
'''
import os
import shutil
import subprocess
...
##### Render quantiles ####
def render_pmf(count, indices, images, poses, H, W, intrinsics, lpips_alex, args, render_kwargs_test, \
    embedcam_fn=None, num_quantiles=100):
    far = render_kwargs_test['far']
    ...
    # result_dir = os.path.join(args.ckpt_dir, args.expname, "distributions_test")
    result_dir = os.path.join(args.ckpt_dir, args.expname, "distributions_2")
    os.makedirs(result_dir, exist_ok=True)
    ...
    with torch.no_grad():
        ...
        filename = os.path.join(result_dir, '{:03d}.png'.format(img_idx))
        imageio.imwrite(filename, rgb8)
        
        samples = extras["z_samples"].cpu().numpy()
        ...
        if args.mode == "constant":
            z_vals_plot = .5 * (z_vals[...,1:] + z_vals[...,:-1])
            # z_vals_plot = z_vals
            weights = weights[...,1:-1]
        ...
        ### This is for linear ###
        if args.mode == "linear":
            z_vals = extras["z_vals"][selected_coor[0], selected_coor[1], ...]
            z_vals = torch.cat([torch.ones_like(z_vals[0].unsqueeze(0))*near, z_vals, torch.ones_like(z_vals[0].unsqueeze(0))*far], -1)

            z_vals_plot = .5 * (z_vals[...,1:] + z_vals[...,:-1])
            z_vals_plot = z_vals_plot.cpu().numpy()
        ...
        print(z_vals_plot.shape)
        print(pdf.shape)
        print(cdf.shape)
        # exit()

        # plt.stem(y, linefmt='b-', markerfmt='bo', basefmt='r-')
        pdf_mean = np.mean(pdf)
        pdf_std = np.std(pdf)

        plt.stem(z_vals_plot, pdf, linefmt='b-', markerfmt='bo', basefmt='r-')
        plt.title('PMF mean:'+ str(pdf_mean) + " std:" + str(pdf_std))

        plt.xlabel('t vals')
        plt.ylabel('PMF')
        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) +'_pmf.png'))

        plt.xlim(selected_xlim)
        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) +'_pmf_zoomed.png'))

        print("Done plotting pmf.")


        plt.clf()
        plt.plot(z_vals_plot, cdf, 'bo')
        plt.title('CDF')
        plt.xlabel('t vals')
        plt.ylabel('CDF')
        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) +'_cdf.png'))

        plt.xlim(selected_xlim)
        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) +'_cdf_zoomed.png'))
        ...
        plt.title('Samples mean:'+ str(samples_mean) + " std:" + str(samples_std))
        plt.xlabel('samples')
        # plt.ylabel('Samples')


        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) + '_with_samples.png'))

        plt.xlim(selected_xlim)
        plt.savefig(os.path.join(result_dir, str(selected_coor[0]) + "_" + str(selected_coor[1]) + '_with_samples_zoomed.png'))
...
def render_rays(ray_batch,
    ...
    ret = {'rgb_map' : rgb_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'depth_map' : depth_map, 'z_vals' : z_vals, 'weights' : weights, "z_samples": z_samples}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths):
    ...
    for i in trange(start, N_iters):
        ...
        if args.i_img != 1:
            ...
            # render
            rgb, _, _, extras = render(H, W, None, chunk=args.chunk, rays=batch_rays, verbose=i < 10, retraw=True, quad_solution_v2=args.quad_solution_v2,\
            zero_tol=args.zero_tol, epsilon=args.epsilon, **render_kwargs_train)
...
def config_parser():
    ...
    parser.add_argument("--visu_quantile_split", type=str, default="test", help='whether to visualize train/test images"')
...
def run_nerf():
    ...
    if args.task == "train":
        ...
        tmp_visu_quantile_split = args.visu_quantile_split
        tmp_num_quantiles = args.num_quantiles
        ...
        args.visu_quantile_split = tmp_visu_quantile_split
        args.num_quantiles = tmp_num_quantiles
    ...
    # Load data
    scene_data_dir = os.path.join("nerf_synthetic/", args_loaded.scene_id)
    ...
    split = split - split[0]
    images_test = render_pmf(None, split, images, poses, H, W, intrinsics, lpips_alex, args, \
        render_kwargs_test, num_quantiles=args.num_quantiles)
