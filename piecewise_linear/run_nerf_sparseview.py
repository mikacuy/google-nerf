'''
Feb. 27, 2023
Refactored combining both piecewise constant (vaniila) and piecewise linear (reformulation) options
'''
import os
import shutil
import subprocess
...
from data import create_random_subsets, load_scene, convert_depth_completion_scaling_to_m, \
    convert_m_to_depth_completion_scaling, get_pretrained_normalize, resize_sparse_depth, load_scene_llff, load_scene_blender, load_scene_mika
...
def render_rays(ray_batch,
    ...
    if N_importance == 0:
        ...
        if not is_joint:
            if mode == "linear":
                z_samples, T_below, tau_below, bin_below, u = sample_pdf_reformulation_return_u(z_vals, weights, tau, T, near, far, N_samples, det=(perturb==0.), pytest=pytest)
            elif mode == "constant":
                z_samples, u = sample_pdf_return_u(z_vals_mid, weights[...,1:-1], N_samples, det=(perturb==0.), pytest=pytest)
        else:
            if mode == "linear":
                z_samples, T_below, tau_below, bin_below, u = sample_pdf_reformulation_joint_return_u(z_vals, weights, tau, T, near, far, N_samples, det=(perturb==0.), pytest=pytest)
            elif mode == "constant":
        ...
        if not is_joint:
            if mode == "linear":
                z_samples, T_below, tau_below, bin_below, u = sample_pdf_reformulation_return_u(z_vals, weights, tau, T, near, far, N_importance, det=(perturb==0.), pytest=pytest, load_u=cached_u)
            elif mode == "constant":
                z_samples, u = sample_pdf_return_u(z_vals_mid, weights[...,1:-1], N_importance, det=(perturb==0.), pytest=pytest, load_u=cached_u)
        else:
            if mode == "linear":
                z_samples, T_below, tau_below, bin_below, u = sample_pdf_reformulation_joint_return_u(z_vals, weights, tau, T, near, far, N_importance, det=(perturb==0.), pytest=pytest, load_u=cached_u)
            elif mode == "constant":
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, scales_init=None, shifts_init=None):
    ...
    for i in trange(start, N_iters):
        ...
        rgb, _, _, extras = render_hyp(H, W, None, chunk=args.chunk, rays=batch_rays, verbose=i < 10, retraw=True,  is_joint=args.is_joint, scale_sample_gradient = args.scale_sample_gradient, **render_kwargs_train)
