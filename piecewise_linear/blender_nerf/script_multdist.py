...
parser.add_argument('--randdist', action="store_true", default=False,
                    help='RAndomly sampled distanes.')

# Split
parser.add_argument('--train_only', action="store_true", default=False,
                    help='Doing training data only.')
...
RESULTS_PATH = 'nerf_dataset/%s_multidist_nv%d_dist%s-%s-%s' \
...
for split in ["train", "test", "val"]:
    os.makedirs(os.path.join(RESULTS_PATH, split), exist_ok=True)
...
for split, nviews in split_to_nviews.items():
    ...
    for i in range(0, nviews):
        ...
        for smp_r in sample_ratios:
            ...
            if RANDOM_VIEWS:
                ...
                rfpath = '{}/r_{:4f}_'.format(split, smp_r) + str(i)
                b_empty.rotation_euler = rot
            else:
                print("Rotation {}, {}".format((stepsize * i), radians(stepsize * i)))
                # scene.render.filepath = fp + '/dist_{}/r_{}_{0:03d}'.format(smp_r, smp_r, int(i * stepsize))
                rfpath =  '{}/r_{:4f}_{0:03d}'.format(split, smp_r, int(i * stepsize))
    ...
    if not DEBUG:
        with open(fp + '/' + 'transforms_%s.json' % split, 'w') as out_file:
