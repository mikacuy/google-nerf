...
def raw2outputs(raw, z_vals, near, far, rays_d, raw_noise_std=0, pytest=False, white_bkgd=False):
...
def compute_samples_around_depth(raw, z_vals, rays_d, N_samples, perturb, lower_bound, near, far):
    sampling_depth, sampling_std = raw2depth(raw, near, far, z_vals, rays_d)
...
def forward_with_additonal_samples(z_vals, near, far, raw, z_vals_2, rays_o, rays_d, viewdirs, embedded_cam, network_fn, network_query_fn, raw_noise_std, pytest):
...
def render_rays(ray_batch,
    ...
    # test time: use precomputed samples along the whole ray and additionally sample around the predicted depth from the first half of samples
        ...
        return forward_with_additonal_samples(z_vals, near, far, raw, z_vals_2, rays_o, rays_d, viewdirs, embedded_cam, network_fn, network_query_fn, raw_noise_std, pytest)
    ...
    raw = network_query_fn(pts, viewdirs, embedded_cam, network_fn)
    rgb_map, disp_map, acc_map, weights, depth_map, tau, T = raw2outputs(raw, z_vals, near, far, rays_d, raw_noise_std, pytest=pytest, white_bkgd=white_bkgd)
