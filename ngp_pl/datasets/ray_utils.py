...
@torch.cuda.amp.autocast(dtype=torch.float32)
def get_ray_directions_scannet(H, W, K, random=False, return_uv=False, flatten=True):
    """
    Get ray directions for all pixels in camera coordinate [right down front].
    Reference: https://www.scratchapixel.com/lessons/3d-basic-rendering/
               ray-tracing-generating-camera-rays/standard-coordinate-systems
...
@torch.cuda.amp.autocast(dtype=torch.float32)
def get_ray_directions_scannet_v2(H, W, K, random=False, return_uv=False, flatten=True):
    """
    Get ray directions for all pixels in camera coordinate [right down front].
    Reference: https://www.scratchapixel.com/lessons/3d-basic-rendering/
               ray-tracing-generating-camera-rays/standard-coordinate-systems
...
def get_rays(directions, c2w):
    ...
    if c2w.ndim==2:
        ...
        ## Original nerf code
        rays_d = torch.bmm(rearrange(directions, 'n c -> n 1 c'), c2w.unsqueeze(0).repeat(directions.shape[0], 1, 1)[..., :3].permute(0,2,1))
        
        # rays_d = torch.bmm(rearrange(directions, 'n c -> n 1 c'), c2w.unsqueeze(0).repeat(directions.shape[0], 1, 1)[..., :3])
        ...
        ## Original nerf code
        rays_d = torch.bmm(rearrange(directions, 'n c -> n 1 c'), c2w[..., :3].permute(0,2,1)) 
        
        # rays_d = torch.bmm(rearrange(directions, 'n c -> n 1 c'), c2w[..., :3])
        rays_d = rearrange(rays_d, 'n 1 c -> n c')
