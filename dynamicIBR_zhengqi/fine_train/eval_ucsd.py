''''
CUDA_VISIBLE_DEVICES=0,1 python eval_ucsd.py --config configs_ucsd/eval_ucsd_scene003.txt
CUDA_VISIBLE_DEVICES=2,3 python eval_ucsd.py --config configs_ucsd/eval_ucsd_scene007.txt
CUDA_VISIBLE_DEVICES=0,1 python eval_ucsd.py --config configs_ucsd/eval_ucsd_scene008.txt
CUDA_VISIBLE_DEVICES=2,3 python eval_ucsd.py --config configs_ucsd/eval_ucsd_scene016.txt
...
VIZ_DEBUG = False
ucsd_mapping = {0:0, 1:1, 2:2, 3:6, 4:5, 5:4, 6:3, 7:7, 8:8, 9:9, 10:6, 11:5, 12:4, 13:3}
...
class DynamicVideoDataset(Dataset):
    def __init__(self, 
        ...
        for i, scene in enumerate(scenes):
            ...
            if np.max(bds) < 10:
                far_depth = min(50, np.max(bds) + 15.)
            else:
    ...
    def __getitem__(self, idx): # idx corresponds to pose index
        render_pose = self.render_poses[ucsd_mapping[idx]]
        intrinsics = self.render_intrinsics[ucsd_mapping[idx]]
        depth_range = self.render_depth_range[ucsd_mapping[idx]]

        train_set_id = self.render_train_set_ids[ucsd_mapping[idx]]
        ...
        for static_pose_id in static_pose_ids:
            ...
            if np.max(np.abs(train_poses[static_pose_id, ...] - train_poses[self.render_idx, ...])) < 1e-6:
                continue

            static_id_dict[ucsd_mapping[static_pose_id % num_imgs_per_cycle]].append(static_pose_id)
...
if __name__ == '__main__':
    ...
    for img_i in (range(3, args.num_frames-3)):#args.num_frames-3):
        ...
        for i, data in enumerate(test_loader):
            ...
            print("img_i ", img_i, ' cam_i ', i)

            if ucsd_mapping[img_i % cycles_num] == i:
                continue
            ...
            if False:
                ...
                # imageio.imwrite(os.path.join('st_%03d_%03d.png'%(img_i, i)), fine_pred_rgb_st)
                # imageio.imwrite(os.path.join('dy_%03d_%03d.png'%(img_i, i)), fine_pred_rgb_rgb)

                # coarse_pred_depth = ret['outputs_fine_ref']['depth'].detach().cpu()
                # coarse_pred_depth_colored = colorize_np(coarse_pred_depth, cmap_name='jet',
                                                        # range=tuple(data['depth_range'].squeeze().numpy()))

                # imageio.imwrite(os.path.join(out_scene_dir, 'depth_debug', '{}.png'.format(i)),
            ...
            dynamic_mask_path = os.path.join(test_dataset.scene_path, 
                                            'mv_masks', 
                                            '%08d'%img_i, 
                                            'cam%02d.png'%(i + 1)) # there is one offset for mask index
