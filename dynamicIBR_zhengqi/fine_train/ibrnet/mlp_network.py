...
class IBRNetDynamic(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        super(IBRNetDynamic, self).__init__()
        self.args = args
        ...
        t_num_freqs = 10
        self.t_embed = PeriodicEmbed(max_freq=t_num_freqs, N_freq=t_num_freqs, linspace=False).float()
        dir_num_freqs = 4
        self.dir_embed = PeriodicEmbed(max_freq=dir_num_freqs, N_freq=dir_num_freqs, linspace=False).float()

        pts_num_freqs = 5
        self.pts_embed = PeriodicEmbed(max_freq=pts_num_freqs, N_freq=pts_num_freqs, linspace=False).float()

        self.n_samples = n_samples
        self.ray_dir_fc = nn.Sequential(nn.Linear(t_num_freqs * 2 + 1, 256),
                                        activation_func,
                                        nn.Linear(256, in_feat_ch + 3),
                                        activation_func)

        self.base_fc = nn.Sequential(nn.Linear((in_feat_ch+3)*3, 256),
                                     activation_func,
                                     nn.Linear(256, 128),
                                     activation_func)

        self.vis_fc = nn.Sequential(nn.Linear(128, 128),
                                    activation_func,
                                    nn.Linear(128, 128+1),
                                    activation_func,
                                    )

        self.vis_fc2 = nn.Sequential(nn.Linear(128, 128),
                                     activation_func,
                                     nn.Linear(128, 1),
                                     nn.Sigmoid()
                                     )

        self.geometry_fc = nn.Sequential(nn.Linear(128*2+1, 256),
                                         activation_func,
                                         nn.Linear(256, 128),
                                         activation_func)
        ...
        self.ref_pts_fc = nn.Sequential(nn.Linear(num_c_xyz + 128, 256), 
                                        activation_func,
                                        nn.Linear(256, 128),
                                        activation_func)

        self.out_geometry_fc = nn.Sequential(nn.Linear(128, 128),
                                             activation_func,
                                             nn.Linear(128, 1))


        if self.input_dir:
            self.rgb_fc = nn.Sequential(nn.Linear(128 +(dir_num_freqs * 2 + 1) * 3, 128),
                                        activation_func,
                                        nn.Linear(128, 64),
                                        activation_func,
                                        nn.Linear(64, 3),
            ...
            self.rgb_fc = nn.Sequential(nn.Linear(32+1, 32),
                                        activation_func,
                                        nn.Linear(32, 16),
                                        activation_func,
                                        nn.Linear(16, 1))
    ...
    def forward(self, 
        ...
        time_pe = self.t_embed(time)[..., None, :].repeat(1, 1, num_views, 1).float()

        # input_ray_t = torch.cat([time_pe, time_diff, ray_diff], dim=-1)
        ...
        if self.input_dir:
            ...
            h = torch.cat([globalfeat, glb_ray_dir_pe[:, None, :].repeat(1, globalfeat.shape[1], 1)], dim=-1)
        else:
            h = globalfeat
...
class IBRNetStatic(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        super(IBRNetStatic, self).__init__()
        self.args = args
        self.anti_alias_pooling = True#args.anti_alias_pooling
        self.input_dir = args.input_dir
        self.input_xyz = args.input_xyz
        ...
        ray_num_freqs = 5
        self.ray_embed = PeriodicEmbed(max_freq=ray_num_freqs, N_freq=ray_num_freqs, linspace=False)
        pts_num_freqs = 5
        self.pts_embed = PeriodicEmbed(max_freq=pts_num_freqs, N_freq=pts_num_freqs, linspace=False)
        ...
        self.ray_dir_fc = nn.Sequential(nn.Linear(4 + num_c_xyz + num_c_ray, 256),
                                        activation_func,
                                        nn.Linear(256, in_feat_ch + 3))

        self.ref_feature_fc = nn.Sequential(nn.Linear(num_c_ray, in_feat_ch+3))

        self.base_fc = nn.Sequential(nn.Linear((in_feat_ch+3)*6, 256),
                                     activation_func,
                                     nn.Linear(256, 128),
                                     activation_func)
        ...
        if self.input_dir:
            self.rgb_fc = nn.Sequential(nn.Linear(128 * 2 + 1 + 4, 128),
                                        activation_func,
                                        nn.Linear(128, 64),
                                        activation_func,
                                        nn.Linear(64, 1))
    ...
    def forward(self, 
        ...
        ref_features = ref_rays_pe[:, None, None, :].expand(-1, src_rays_pe.shape[1], src_rays_pe.shape[2], -1)
        src_features = torch.cat([pts_pe.unsqueeze(2).expand(-1, -1, src_rays_pe.shape[2], -1), src_rays_pe], dim=-1)
        ...
        if self.input_dir:
            x = torch.cat([globalfeat[:, :, None, :].expand(-1, -1, x.shape[2], -1), x, vis, ray_diff], dim=-1)
        else:
            raise ValueError
...
class MotionMLP(nn.Module):
    def __init__(self, 
        ...
        self.coeff_linear.weight.data.fill_(0.)
        self.coeff_linear.bias.data.fill_(0.)
