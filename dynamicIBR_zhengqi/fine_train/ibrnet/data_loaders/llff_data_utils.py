...
def load_llff_data(basedir, height, num_avg_imgs, 
                   factor=8, render_idx=8, 
                   recenter=True, bd_factor=.75,
                   spherify=False, path_zflat=False, 
                   load_imgs=True):

    out = _load_data(basedir, factor=None, load_imgs=load_imgs, height=height)  # factor=8 downsamples original imgs by 8x
    ...
    if FAR_SCALE:
        scale = 1. if bd_factor is None else 1. / (bds.max())
    else: # same as NeRF
...
def render_wander_path(c2w):
    ...
    for i in range(num_frames):
        ...
        z_trans = 0.#max_trans * np.cos(2.0 * np.pi * float(i) / float(num_frames)) / 3.0
