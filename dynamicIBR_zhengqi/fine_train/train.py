'''
python train.py --config configs_ucsd/train_ucsd_scene003.txt
python train.py --config configs_ucsd/train_ucsd_scene007.txt
python train.py --config configs_ucsd/train_ucsd_scene008.txt
python train.py --config configs_ucsd/train_ucsd_scene010.txt
python train.py --config configs_ucsd/train_ucsd_scene16.txt
python train.py --config configs_ucsd/train_ucsd_scene14.txt
python train.py --config configs_ucsd/train_ucsd_scene016.txt
python train.py --config configs_ucsd/train_ucsd_scene06.txt
python train.py --config configs_ucsd/train_ucsd_scene026.txt


python train.py --config configs_nvidia/train_jumping_long.txt
python train.py --config configs_nvidia/train_skating_long.txt
python train.py --config configs_nvidia/train_balloon1_long.txt
python train.py --config configs_nvidia/train_balloon2_long.txt
python train.py --config configs_nvidia/train_playground_long.txt
python train.py --config configs_nvidia/train_truck_long.txt
...
def train(args):
    ...
    '_an_cycle-%d'%args.anneal_cycle + \
    '_w-distortion-%.4f'%args.w_distortion + \
    '_w-se-%.4f'%(args.w_skew_entropy) + \
    '_occ_w_%d'%(args.occ_weights_mode) + \
    'mask-st_%d'%args.mask_static
    ...
    for epoch in range(start_epoch, int(10**5)):
        ...
        for ii, train_data in enumerate(train_loader):
            ...
            ref_time_offset = [int(near_idx - ref_frame_idx) for near_idx in nearest_pose_ids] 
            ...
            num_dy_views = len(ref_time_offset)
            ...
            cb_featmaps_1_fine, _ = model.feature_net_fine(cb_src_rgbs)
            ref_featmaps_fine, anchor_featmaps_fine = cb_featmaps_1_fine[0:num_dy_views], cb_featmaps_1_fine[num_dy_views:]

            if args.mask_static:
                # print(" ray_batch['static_src_masks'] ",  ray_batch['static_src_masks'])
                # sys.exit()
            ...
            ret = render_rays(frame_idx=(ref_frame_idx, anchor_frame_idx),
                              ...
                              model=model,
                              projector=projector,
                              coarse_featmaps=(ref_featmaps, anchor_featmaps, static_featmaps),
                              fine_featmaps=(ref_featmaps_fine, anchor_featmaps_fine, static_featmaps_fine),
            ...
            # # RGB loss
            rgb_loss = rgb_criterion(ret['outputs_fine_ref'], ray_batch, use_motion_mask=False)
            rgb_loss += compute_temporal_rgb_loss(ret['outputs_fine_anchor'], ray_batch, use_motion_mask=False)

            # divsor = global_step // (args.decay_base * 1000)
            divsor = epoch // args.init_decay_epoch

            # RGB loss for dynamic component only
            dynamic_rgb_decay_rate = 5.
            rgb_loss += rgb_criterion(ret['outputs_fine_ref_dy'], ray_batch, use_motion_mask=True) / ( (dynamic_rgb_decay_rate) ** divsor) 
            rgb_loss += compute_temporal_rgb_loss(ret['outputs_fine_anchor_dy'], ray_batch, use_motion_mask=True) / ( (dynamic_rgb_decay_rate) ** divsor) 

            # disparity loss
            w_disp = args.w_disp / (decay_rate ** divsor) 
            pred_disp = 1. / torch.clamp(ret['outputs_fine_ref']["depth"], min=1e-2)

            gt_disp = ray_batch['disp']
            pred_mask = ret['outputs_fine_ref']["mask"].detach()
            ...
            flow_loss = w_flow * compute_flow_loss(ret['outputs_fine_ref']["render_flows"], 
                                                   ray_batch['flows'], 
                                                   flow_mask)
    
            # traj consistency loss
            if args.anneal_cycle: # find annealing better 
                w_cycle = min(0.5, args.w_cycle + divsor * 0.1)
            else:
                w_cycle = args.w_cycle

            pts_traj_anchor = ret['outputs_fine_anchor']['pts_traj_anchor']
            pts_traj_ref = ret['outputs_fine_anchor']['pts_traj_ref']

            occ_weights = ret['outputs_fine_anchor']['occ_weights'][None, ..., None].repeat(pts_traj_anchor.shape[0], 
                                                                                              1, 1, 
            ...
            reg_loss = args.w_sf_zero * torch.mean(torch.abs((ret['outputs_fine_anchor']['sf_seq'])))
            # temporal smooth loss
            reg_loss += args.w_sf_sm * 0.5 * torch.mean(torch.pow(ret['outputs_fine_anchor']['sf_seq'][:-1] \
                - ret['outputs_fine_anchor']['sf_seq'][1:], 2))
            # spatial smooth loss
            reg_loss += args.w_sf_sm * torch.mean(torch.abs(ret['outputs_fine_anchor']['sf_seq'][:, :, 1:, :] \
                - ret['outputs_fine_anchor']['sf_seq'][:, :, :-1, :]))

            # skew entropy loss
            render_weights_dy = torch.sum(ret['outputs_fine_ref']['weights_dy'], dim=-1)
            render_weights_st = torch.sum(ret['outputs_fine_ref']['weights_st'], dim=-1)
            ...
            # distortion loss used in mip360
            s_vals = ret['outputs_fine_ref']['s_vals']
            mid_dist = (s_vals[:, 1:] + s_vals[:, :-1]) * 0.5
            interval = (s_vals[:, 1:] - s_vals[:, :-1])
            w_distortion = args.w_distortion
            distortion_loss = w_distortion * eff_distloss_native(ret['outputs_fine_ref']['weights'][:, :-1], 
                                                                 mid_dist, 
                                                                 interval)
            # static mask reg
            if args.sfm_reg:
                static_sfm_mask = 1. - ray_batch['sfm_mask'].float()
                static_sfm_mask *= ret['outputs_fine_ref']['mask'].float()
                sfm_loss = compute_rgb_loss(ret['outputs_fine_ref']['rgb_static'], ray_batch, static_sfm_mask) #/ (decay_rate ** divsor)
            else:
                sfm_loss = torch.zeros(1).cuda()

            loss = cycle_loss + flow_loss + disp_loss + rgb_loss + \
                   reg_loss + skew_entropy_loss + \
                   distortion_loss + sfm_loss
            ...
            if args.local_rank == 0:
                if global_step % args.i_print == 0:
                    ...
                    mse_error = img2mse(ret['outputs_fine_ref']['rgb'], ray_batch['rgb']).item()
                ...
                if global_step % args.i_img == 0:
                    ...
                    gt_img = tmp_ray_train_sampler.rgb.reshape(H, W, 3)
                    gt_disp = tmp_ray_train_sampler.disp.reshape(H, W, 1)
                    log_view_to_tb(writer, global_step, args, num_dy_views,
                                   model, tmp_ray_train_sampler, projector,
                                   gt_img, gt_disp,
                                   frame_idx=(ref_frame_idx, anchor_frame_idx),
                                   time_embedding=(ref_time_embedding, anchor_time_embedding),
...
def log_view_to_tb(writer, global_step, args, 
    ...
    with torch.no_grad():
        ...
        if DEBUG:
            ...
            # for ii in range(gt_flows.shape[0]):
                # print(ii)
                # combined_flow = np.concatenate([flow_to_image(gt_flows[ii].cpu().numpy())/255., flow_to_image(ret['outputs_fine_ref']['render_flows'][ii].cpu().numpy())/255.], axis=1)
                # imageio.imwrite('viz_flow_%3d.png'%ii, combined_flow)

            ref_rgb_render = ret['outputs_fine_ref']['rgb'].detach().cpu().numpy()
            ref_depth_render = ret['outputs_fine_ref']['depth'].detach().cpu().numpy()
            ...
            # plt.subplot(2, 3, 4)
            # plt.imshow(ret['outputs_fine']["depth"].cpu().numpy(), cmap='gray')
            plt.tight_layout()
        ...
        # exp_sf = ret['outputs_fine_anchor']['exp_sf'].detach().cpu()
        ...
        # occ_weight_map = ret['outputs_fine_anchor']['occ_weight_map'].detach().cpu()
    ...
    ref_rgb_pred = img_HWC2CHW(ret['outputs_fine_ref']['rgb'].detach().cpu())
    static_rgb_pred = img_HWC2CHW(ret['outputs_fine_ref']['rgb_static'].detach().cpu())
    dy_rgb_pred = img_HWC2CHW(ret['outputs_fine_ref']['rgb_dy'].detach().cpu())

    anchor_rgb_pred = img_HWC2CHW(ret['outputs_fine_anchor']['rgb'].detach().cpu())
    ref_rgb_pred_coarse = img_HWC2CHW(ret['outputs_coarse_ref']['rgb'].detach().cpu())

    gt_flows = ray_batch['flows'].reshape(ray_batch['flows'].shape[0], gt_img.shape[0], gt_img.shape[1], 2)

    exp_sf = ret['outputs_fine_ref']['exp_sf'].detach().cpu()
    exp_sf_mag = torch.norm(exp_sf, dim=-1) 
    ...
    if False:
        ...
        # print(torch.median(torch.norm(ret['outputs_fine']['render_fwd_flows'][0].cpu() - gt_fwd_flows[0].cpu(), dim=-1)))
        # print(torch.median(torch.norm(ret['outputs_fine']['render_bwd_flows'][0].cpu() - gt_bwd_flows[0].cpu(), dim=-1)))

        # print(torch.median(torch.norm(ret['outputs_fine']['render_fwd_flows'][1].cpu() - gt_fwd_flows[1].cpu(), dim=-1)))
        # print(torch.median(torch.norm(ret['outputs_fine']['render_bwd_flows'][1].cpu() - gt_bwd_flows[1].cpu(), dim=-1)))

        # print(torch.median(torch.norm(ret['outputs_fine']['render_fwd_flows'][2].cpu() - gt_fwd_flows[2].cpu(), dim=-1)))
        # print(torch.median(torch.norm(ret['outputs_fine']['render_bwd_flows'][2].cpu() - gt_bwd_flows[2].cpu(), dim=-1)))

        # plt.figure(figsize=(19, 6))
        # plt.subplot(2, 2, 1)
        # plt.imshow(gt_img.cpu().numpy())
        # plt.subplot(2, 2, 2)
        # plt.imshow(ret['outputs_fine']['rgb'].cpu().numpy()) 
        # plt.subplot(2, 2, 3)
        # plt.imshow(gt_disp[..., 0].cpu().numpy(), cmap='gray')
        # plt.subplot(2, 2, 4)
        # plt.imshow(ret['outputs_fine']["depth"].cpu().numpy(), cmap='gray')
        ...
        # plt.figure(figsize=(19, 6))
        # plt.subplot(2, 3, 1)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_fwd_flows'][0].cpu().numpy())/255.)
        # plt.subplot(2, 3, 4)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_bwd_flows'][0].cpu().numpy())/255.)

        # plt.subplot(2, 3, 2)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_fwd_flows'][1].cpu().numpy())/255.)
        # plt.subplot(2, 3, 5)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_bwd_flows'][1].cpu().numpy())/255.)

        # plt.subplot(2, 3, 3)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_fwd_flows'][2].cpu().numpy())/255.)
        # plt.subplot(2, 3, 6)
        # plt.imshow(flow_to_image(ret['outputs_fine']['render_bwd_flows'][2].cpu().numpy())/255.)
    ...
    ref_depth_im = ret['outputs_fine_ref']['depth'].detach().cpu()
    anchor_depth_im = ret['outputs_fine_anchor']['depth'].detach().cpu()
    occ_weight_map = ret['outputs_fine_anchor']['occ_weight_map'].detach().cpu()

    writer.add_image(prefix + 'ref_rgb_pred_coarse', torch.clamp(ref_rgb_pred_coarse, 0., 1.), global_step)

    writer.add_image(prefix + 'render_rgb_fine_ref', torch.clamp(ref_rgb_pred, 0., 1.), global_step)
    writer.add_image(prefix + 'render_rgb_fine_anchor', torch.clamp(anchor_rgb_pred, 0., 1.), global_step)
    ...
    writer.add_image(prefix + 'render_depth_fine', render_depth_ref, global_step)
    writer.add_image(prefix + 'anchor_depth_fine', render_depth_anchor, global_step)
    ...
    writer.add_image(prefix + 'gt_disp_fine', gt_disp_viz, global_step)
    writer.add_image(prefix + 'gt_rgb_fine', rgb_gt, global_step)

    # write flow
    rd_flow_stack = []
    gt_flow_stack = []
    for ii in range(gt_flows.shape[0]):
        rd_flow_stack.append(torch.Tensor(flow_to_image(ret['outputs_fine_ref']['render_flows'][ii].cpu().numpy())/255.))
