...
class Projector():
    ...
    def normalize(self, pixel_locations, h, w):
        resize_factor = torch.tensor([w-1., h-1.]).to(self.device)[None, None, :]
    ...
    def compute_projections_motions(self, xyz, train_cameras):
        '''
        project 3D points into cameras
        :param xyz: [n_views, ..., 3]
        :param train_cameras: [n_views, 34], 34 = img_size(2) + intrinsics(16) + extrinsics(16)
        ...
        # print("train_intrinsics ", train_intrinsics.shape)
        # print("train_poses ", train_poses.shape)
        # print("xyz_h ", xyz_h.shape)

        projections = train_intrinsics.bmm(torch.inverse(train_poses)) \
            .bmm(xyz_h.permute(0, 2, 1)) # [n_views, 4, n_points]
        ...
        # print("pixel_locations ", pixel_locations.shape)
        # print("projections ", projections.shape)

        # sys.exit()

        mask = projections[..., 2] > 0   # a point is invalid if behind the camera
        return pixel_locations.reshape((num_views, ) + original_shape[1:] + (2, )), \
               mask.reshape((num_views, ) + original_shape[1:])


    def compute_angle(self, xyz, query_camera, train_cameras):
        '''
        :param xyz: [n_views, n_rays, n_samples, 3]
    ...
    def compute_angle_motions(self, xyz_st, xyz, query_camera, train_cameras):
        '''
        :param xyz: [n_views, n_rays, n_samples, 3]
        :param query_camera: [34, ]
        :param train_cameras: [n_views, 34]
        ...
        ray2tar_pose = F.normalize(query_pose[:, :3, 3].unsqueeze(1) - xyz_st_, dim=-1)
        ray2train_pose = F.normalize(train_poses[:, :3, 3].unsqueeze(1) - xyz_, dim=-1)
        # ray2train_pose /= (torch.norm(ray2train_pose, dim=-1, keepdim=True) + 1e-6)
        ray_diff = ray2tar_pose - ray2train_pose

        # print("ray_diff ", ray_diff.sh)
        # ray_diff_norm = torch.norm(ray_diff, dim=-1, keepdim=True)
        ray_diff_dot = torch.sum(ray2tar_pose * ray2train_pose, dim=-1, keepdim=True)
        ray_diff_direction = F.normalize(ray_diff, dim=-1) #ray_diff / torch.clamp(ray_diff_norm, min=1e-6)
    ...
    def compute_with_motions(self, xyz_st, xyz, query_camera, train_imgs, train_cameras, featmaps):
        '''
        :param xyz: [n_source_views, n_rays, n_samples, 3]
        :param query_camera: [1, 34], 34 = img_size(2) + intrinsics(16) + extrinsics(16)
        :param train_imgs: [1, n_views, h, w, 3]
        :param train_cameras: [1, n_views, 34]
        :param featmaps: [n_views, d, h, w]
        ...
        # compute the projection of the query points to each reference image
        pixel_locations, mask_in_front = self.compute_projections_motions(xyz, train_cameras)
        ...
        # mask
        inbound = self.inbound(pixel_locations, h, w)
        ray_diff = self.compute_angle_motions(xyz_st, xyz, query_camera, train_cameras).detach()

        ray_diff = ray_diff.permute(1, 2, 0, 3)
        mask = (inbound * mask_in_front).float().permute(1, 2, 0)[..., None]   # [n_rays, n_samples, n_views, 1]
        
        return rgb_feat_sampled, ray_diff, mask #ray_diff, mask
