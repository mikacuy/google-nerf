...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.dy_mask = data['dy_mask'] if 'dy_mask' in data.keys() else None
        ...
        if self.dy_mask is not None:
            self.dy_mask = self.dy_mask.reshape(-1, 1)
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        if self.rgb is not None:
            ...
            dy_mask = self.dy_mask[select_inds].squeeze()
        ...
        ret = {'ray_o': rays_o.to(self.device),
               ...
               "dy_mask": dy_mask.to(self.device),
