'''
CUDA_VISIBLE_DEVICES=2,3 python train_ucsd.py --config configs_ucsd/train_ucsd_scene06.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene003.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene007.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene008.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene010.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene16.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene14.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene016.txt
python train_ucsd.py --config configs_ucsd/train_ucsd_scene026.txt
...
def train(args):
    ...
    for epoch in range(start_epoch, int(10**5)):
        ...
        for ii, train_data in enumerate(train_loader):
            ...
            # # RGB loss
            rgb_loss = rgb_criterion(ret['outputs_coarse_ref'], ray_batch, use_motion_mask=False)
            rgb_loss += compute_temporal_rgb_loss(ret['outputs_coarse_anchor'], ray_batch, use_motion_mask=False)

            divsor = epoch // args.init_decay_epoch

            # RGB loss for dynamic regions only
            dynamic_rgb_decay_rate = 5.
            if epoch < (args.init_decay_epoch * 0.25):
                dynamic_mask = ret['outputs_coarse_ref']['mask'].float() * ray_batch['dy_mask'].float()
                rgb_loss += compute_rgb_loss(ret['outputs_coarse_ref']['rgb_dy'], ray_batch, dynamic_mask) 

            # RGB loss for dynamic component only
            rgb_loss += rgb_criterion(ret['outputs_coarse_ref_dy'], ray_batch, use_motion_mask=True) / ( (dynamic_rgb_decay_rate) ** divsor) 
            rgb_loss += compute_temporal_rgb_loss(ret['outputs_coarse_anchor_dy'], ray_batch, use_motion_mask=True) / ( (dynamic_rgb_decay_rate) ** divsor) 
...
def train(args):
    ...
    # Create criterion
    rgb_criterion = Criterion()
    tb_dir = os.path.join(args.rootdir, 'logs_ucsd/', args.expname)
    if args.local_rank == 0:
        writer = SummaryWriter(tb_dir)
        print('saving tensorboard files to {}'.format(tb_dir))
    scalars_to_log = {}
    ...
    for epoch in range(start_epoch, int(10**5)):
        ...
        for ii, train_data in enumerate(train_loader):
            ...
            w_reg = args.w_reg
            # minimal scene flow loss
            reg_loss = w_reg * torch.mean(torch.abs((ret['outputs_coarse_anchor']['sf_seq'])))
            # temporal smooth loss
            reg_loss += w_reg * 0.5 * torch.mean(torch.pow(ret['outputs_coarse_anchor']['sf_seq'][:-1] \
                - ret['outputs_coarse_anchor']['sf_seq'][1:], 2))
            # spatial smooth loss
            # reg_loss += w_reg * torch.mean(torch.abs(ret['outputs_coarse_anchor']['sf_seq'][:, :, 1:, :] \
                # - ret['outputs_coarse_anchor']['sf_seq'][:, :, :-1, :]))
