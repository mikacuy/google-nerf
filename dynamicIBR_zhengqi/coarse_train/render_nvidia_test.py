...
class DynamicVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for static_pose_id in static_pose_ids:
            if static_pose_id % num_imgs_per_cycle in nearest_pose_ids_mod or static_pose_id % num_imgs_per_cycle == idx % num_imgs_per_cycle:
                continue
        ...
        for key in static_id_dict:
            if abs(static_id_dict[key][0] - idx) < abs(static_id_dict[key][1] - idx):
                static_pose_ids.append(static_id_dict[key][0])
            else:
                static_pose_ids.append(static_id_dict[key][1])
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(test_loader):
        ...
        ref_time_offset = [int(near_idx - ref_frame_idx) for near_idx in data['nearest_pose_ids'].squeeze().tolist()[:NUM_DYNAMIC_SRC_VIEWS]]
        ...
        with torch.no_grad():
            ...
            # ref_featmaps, _ = model.feature_net(ray_batch['src_rgbs'].squeeze(0).permute(0, 3, 1, 2)[:NUM_DYNAMIC_SRC_VIEWS])
            # static_featmaps, _ = model.feature_static_net(ray_batch['src_rgbs'].squeeze(0).permute(0, 3, 1, 2))
            ref_featmaps, _ = model.feature_net(ray_batch['src_rgbs'].squeeze(0).permute(0, 3, 1, 2)[:NUM_DYNAMIC_SRC_VIEWS])
            _, static_featmaps = model.feature_net(ray_batch['src_rgbs'].squeeze(0).permute(0, 3, 1, 2))
