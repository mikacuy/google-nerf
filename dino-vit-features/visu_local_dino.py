...
def plot_pca(pil_image: Image.Image, pca_image: numpy.ndarray, save_dir: str, last_components_rgb: bool = True,
    ...
    pca_image = cv2.resize(pca_image, pil_image.size, interpolation=cv2.INTER_AREA)
    ...
    for comp_idx in range(n_components):
        ...
        comp_range = pca_image[location[0]:location[0]+patch_size, location[1]:location[1]+patch_size, comp_idx]
        # comp_range = pca_image[215:318, 100:225, comp_idx]
        comp_min = comp_range.min(axis=(0, 1))
        comp_max = comp_range.max(axis=(0, 1))

        comp_img = np.maximum(comp - comp_min, 0.) / (comp_max - comp_min)
    ...
    new_im = Image.new('RGB', pil_image.size)
    ...
    if last_components_rgb:
        ...
        comp_range = pca_image[location[0]:location[0]+patch_size, location[1]:location[1]+patch_size, -3:]
        # comp_range = pca_image[215:318, 100:225, comp_idx]
        comp_min = comp_range.min(axis=(0, 1))
        comp_max = comp_range.max(axis=(0, 1))
        ...
        comp_img_patch = comp_img[location[0]:location[0]+patch_size, location[1]:location[1]+patch_size]

        comp_file_path = save_dir / f'{save_prefix}_{comp_idxs}_rgb.png'
        patch_file_path = save_dir / f'{save_prefix}_{comp_idxs}_rgb_patch.png'

        pca_pil = Image.fromarray((comp_img * 255).astype(np.uint8))
        pca_pil_patch = Image.fromarray((comp_img_patch * 255).astype(np.uint8))

        # region = pca_pil[100:225, 215:318]
        new_im.paste(pca_pil_patch, (location[1],location[0]))

        # if save_resized:
        #     pca_pil = pca_pil.resize(pil_image.size, resample=PIL.Image.NEAREST)

        pca_pil.save(comp_file_path)
        new_im.save(patch_file_path)
