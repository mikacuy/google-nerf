...
def pca(image_paths_sets, descriptors_paths_sets, load_size: int = 224, layer: int = 11, facet: str = 'key', bin: bool = False, stride: int = 4,
    ...
    for i in range(len(descriptors_paths_sets)):
      descriptors_path = descriptors_paths_sets[i]
      image_paths = image_paths_sets[i]
      ...
      for image_path in image_paths:
          ...
          output_path = os.path.join(descriptors_path, image_path.name[:-4] + ".pth")
          # descs = extractor.extract_descriptors(image_batch.to(device), layer, facet, bin, include_cls=False)
          descs = torch.load(output_path)
          # print(descs.shape)
          # exit()

          descs = descs.cpu().numpy()

          curr_num_patches, curr_load_size = np.array([134, 239]), np.array([540, 960])
        #   curr_num_patches, curr_load_size = np.array([89, 159]), np.array([360, 640])
...
def plot_pca(pil_image: Image.Image, pca_image: numpy.ndarray, save_dir: str, last_components_rgb: bool = True,
             save_resized=True, save_prefix: str = '', folder: str = ""):
    """
    finding pca of a set of images.
    ...
    pil_image_path = save_dir / f'{save_prefix}_{folder}_orig_img.png'
    ...
    for comp_idx in range(n_components):
        ...
        comp_file_path = save_dir / f'{save_prefix}_{comp_idx}_{folder}.png'
    ...
    if last_components_rgb:
        ...
        comp_max = comp.max(axis=(0, 1))
        comp_img = (comp - comp_min) / (comp_max - comp_min)
        comp_file_path = save_dir / f'{save_prefix}_{comp_idxs}_{folder}_rgb.png'
        pca_pil = Image.fromarray((comp_img * 255).astype(np.uint8))
        if save_resized:
            pca_pil = pca_pil.resize(pil_image.size, resample=PIL.Image.NEAREST)
        pca_pil.save(comp_file_path)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        # prepare directories
        all_root_dirs = ["/home/mikacuy/Desktop/coord-mvs/walking_dvd/mv_images/00000/", "/home/mikacuy/Desktop/coord-mvs/walking_dvd/mv_images/00074/"]

        # all_descriptor_dirs = ["output_dino_frame_00000_ds3x", "output_dino_frame_00074_ds3x"]
        all_descriptor_dirs = ["output_dino_frame_00000_ds2x", "output_dino_frame_00074_ds2x"]

        all_image_paths = []
        all_descriptor_paths = []
        for i in range(len(all_root_dirs)):
          root_dir = Path(all_root_dirs[i])
          images_paths = [x for x in root_dir.iterdir() if x.suffix.lower() in ['.jpg', '.png', '.jpeg']]

          desc_dir = Path(all_descriptor_dirs[i])
          descs_paths = [x for x in desc_dir.iterdir() if x.suffix.lower() in ['.pth']]

          all_image_paths.append(images_paths)
          all_descriptor_paths.append(descs_paths)

        save_dir = Path(args.save_dir)
        save_dir.mkdir(exist_ok=True, parents=True)

        pca_per_image = pca(all_image_paths, all_descriptor_dirs, args.load_size, args.layer, args.facet, args.bin, args.stride, args.model_type,
                            args.n_components, args.all_together, save_dir)

        all_image_paths = all_image_paths[0] + all_image_paths[1]
        # print(all_image_paths)
        print("saving images")
        for image_path, (pil_image, pca_image) in tqdm(zip(all_image_paths, pca_per_image)):
            save_prefix = image_path.stem
            folder = image_path.absolute().as_posix()
            folder = folder.split("/")[-2]

            plot_pca(pil_image, pca_image, str(save_dir), args.last_components_rgb, args.save_resized, save_prefix, folder)
