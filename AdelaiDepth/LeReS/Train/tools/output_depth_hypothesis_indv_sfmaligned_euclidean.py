...
# parser.add_argument('--dump_dir', default= "dump_0117_scene0710_sfmaligned_indv_euclidean/", type=str)
# parser.add_argument('--dump_dir', default= "dump_0117_scene0758_sfmaligned_indv_euclidean/", type=str)
parser.add_argument('--dump_dir', default= "dump_0117_scene0781_sfmaligned_indv_euclidean/", type=str)
# parser.add_argument('--dump_dir', default= "dump_0117_scene0708_sfmaligned_indv_euclidean/", type=str)
# parser.add_argument('--dump_dir', default= "dump_0117_scene0738_sfmaligned_indv_euclidean/", type=str)
    ...
    for i, data in enumerate(zcache_dataloader):
        ...
        for k in range(num_sets):
            ...
            for s in range(mini_batch_size):
                ...
                #### Convert to Euclidean depth ####
                X, Y = np.meshgrid(np.arange(depth_orig_size.shape[1])-cx, np.arange(depth_orig_size.shape[0])-cy)
                positions = np.vstack([(X/fx).ravel(), (Y/fy).ravel()])

                dist_to_center = np.linalg.norm(positions - np.zeros((2,1)), axis=0)
                dist_to_center = dist_to_center.reshape(depth_orig_size.shape)
                # plt.imsave(os.path.join(temp_fol, 'dist_to_center.png'), dist_to_center, cmap='rainbow')
                
                theta = np.arctan(dist_to_center)
                # plt.imsave(os.path.join(temp_fol, 'theta.png'), theta, cmap='rainbow')

                cos_theta = np.cos(theta)
                # plt.imsave(os.path.join(temp_fol, 'cos_theta.png'), cos_theta, cmap='rainbow')
                
                curr_pred_depth_euclidean = np.divide(curr_pred_depth_raw, cos_theta) 

                curr_pred_depth_metric, curr_scale, curr_shift = recover_metric_depth(curr_pred_depth_euclidean, depth_orig_size)
                ...
                ## SfM depth
                curr_pred_depth_euclidean_metric, curr_sfm_scale, curr_sfm_shift = recover_metric_depth(curr_pred_depth_euclidean, sfm_depth_img)

                sfm_image_scales.append(curr_sfm_scale)
                sfm_image_shifts.append(curr_sfm_shift)

                sfm_depth_rmse = compute_rmse(curr_pred_depth_euclidean_metric[valid_depth], depth_orig_size[valid_depth])                

                curr_pp_error = per_pixel_error(curr_pred_depth_euclidean_metric, depth_orig_size)
                ...
                all_pred_depths.append(curr_pred_depth_euclidean_metric)

                if i%10==0  or VISU_ALL:
                    # save depth
                    plt.imsave(os.path.join(temp_fol, img_name+'-depth-euclidean.png'), curr_pred_depth_euclidean_metric, cmap='rainbow')           
                    image_fname.append(os.path.join(temp_fol, img_name+'-depth-euclidean.png'))

                    ### Output point cloud
                    # reconstruct_depth_intrinsics(curr_pred_depth, rgb, pc_fol, img_name, intrinsics)

                    # rgb_orig = cv2.imread(data['A_paths'][0])
                    # reconstruct_depth_intrinsics(curr_pred_depth_euclidean, rgb_orig, pc_fol, img_name+"-sfmscaled", intrinsics)
