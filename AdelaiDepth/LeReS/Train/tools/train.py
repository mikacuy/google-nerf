...
def do_train(train_dataset, val_dataset, train_args,
    ...
    try:
        for epoch in range(MAX_EPOCH):
            ...
            # # validate the model
            # if epoch % 20 == 0 and val_dataloader is not None and epoch != 0:
            #     model.eval()
        ...
        #     print(data['quality_flg'].shape)
        #     print(data['focal_length'].shape)
        #     print(data['planes'].shape)
        ...
        #     ## Hard coded d_latent
        #     z = torch.normal(0.0, 1.0, size=(data['rgb'].shape[0], num_samples, d_latent))
        #     z = z.view(-1, d_latent)
        #     print(z.shape)

        #     ### Repeat for the number of samples
        #     data['rgb'] = data['rgb'].unsqueeze(1).repeat(1,num_samples, 1, 1, 1)
        #     data['rgb'] = data['rgb'].view(-1, C, H, W)
        #     data['depth'] = data['depth'].unsqueeze(1).repeat(1,num_samples, 1, 1, 1)
        #     data['depth'] = data['depth'].view(-1, 1, H, W)            
        #     data['quality_flg'] = data['quality_flg'].unsqueeze(1).repeat(1,num_samples)
        #     data['quality_flg'] = data['quality_flg'].view(-1)
        #     data['focal_length'] = data['focal_length'].unsqueeze(1).repeat(1,num_samples)
        #     data['focal_length'] = data['focal_length'].view(-1)
        #     data['planes'] = data['planes'].unsqueeze(1).repeat(1,num_samples,1,1)
        #     data['planes'] = data['planes'].view(-1, H, W)
        ...
        #     losses_dict, total_raw = out['losses']
        #     print(losses_dict)
        #     print(losses_dict["total_loss"])
        #     print(total_raw.shape)

        #     ### For saving z-values
        #     ### Change shape and get min
...
def main_worker(local_rank: int, ngpus_per_node: int, train_args, val_args):
    ...
    if train_args.load_ckpt:
        if os.path.isfile(train_args.load_ckpt):
            ...
            # model.load_state_dict(strip_prefix_if_present(checkpoint['depth_model'], "module."),
            #                             strict=True)
            ...
            # Still dont know where these come from...
            shift_keys = {k: v for k, v in checkpoint['shift_model'].items()}
            focal_keys = {k: v for k, v in checkpoint['focal_model'].items()}

            print(len(model_dict.keys()))
            print()
            print(len(depth_keys.keys()))
            print(len(shift_keys.keys()))
            print(len(focal_keys.keys()))

            # ### Inspect missing keys
            # delta_keys = {k: v for k, v in model_dict.items() if k not in checkpoint['depth_model']}
            # print(delta_keys.keys())
            # print()
            # print(shift_keys.keys())
            # print()
            # print(focal_keys.keys())            
            # exit()
