...
parser.add_argument("--logdir", default="log_debug_finetune/", help="path to the log directory", type=str)

### Load pretrained model
# parser.add_argument("--pretrained_logdir", default="log_0726_lrfixed_001/", help="path to the log directory", type=str)
# parser.add_argument("--ckpt", default="epoch104_step39375.pth", help="checkpoint", type=str)

parser.add_argument("--pretrained_logdir", default="log_0825_encv2_noaug_noshuffle_s12/", help="path to the log directory", type=str)
parser.add_argument("--ckpt", default="epoch56_step0.pth", help="checkpoint", type=str)

parser.add_argument('--loss_mode', type=str, default='_ranking-edge_msgil-normal_meanstd-tanh_', help='losses to use')
parser.add_argument('--epoch', default= 20, type=int)
parser.add_argument('--batchsize', default= 4, type=int)

parser.add_argument('--base_lr', default= 0.00001, type=float)
parser.add_argument('--mlp_lr', default= 0.00001, type=float)
parser.add_argument('--mlp_lr2', default= 0.00001, type=float)
...
parser.add_argument('--reinit_adain', default=False, type=bool)
...
REINIT_ADAIN = FLAGS.reinit_adain

PRETRAINED_LOGDIR = FLAGS.pretrained_logdir
CKPT = FLAGS.ckpt
...
CKPT_FILE = os.path.join(PRETRAINED_LOGDIR, "ckpt", CKPT)
...
mean0, var0, mean1, var1, mean2, var2, mean3, var3 = load_mean_var_adain(os.path.join(PRETRAINED_LOGDIR, "mean_var_adain.npy"), torch.device("cuda"))
...
train_dataset = FinetuneDataset(datapath, dataset_name, is_nsvf=IS_NSVF, split="train", data_aug=False)
    ...
    if epoch == 0 and REINIT_ADAIN:
        ...
        with torch.no_grad():
            for i, data in enumerate(subset_dataloader):
                ...
                ### If NSVF data need to mask out the background when taking the mean --> not straightforward to init
                # print(adain0.shape)
                # print(adain1.shape)
                # print(adain2.shape)
                # print(adain3.shape)
