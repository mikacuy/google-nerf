...
def recover_depth_scale_shift(pred, gt):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(zcache_dataloader):     
            ...
            for s in range(batch_size):
                ...
                scale, shift = recover_depth_scale_shift(curr_pred_depth, gt_depth)
        ...
        print(all_shifts.shape)


        fname = "train_depth_scales.npy"
        np.save(os.path.join(visu_dir, fname), all_scales)

        fname = "train_depth_shifts.npy"
        np.save(os.path.join(visu_dir, fname), all_shifts)
...
def main():
    ...
    train_args.phase_anno = "train"
    train_args.phase = "test"

    train_args.batchsize = 1
    train_args.thread = 0
