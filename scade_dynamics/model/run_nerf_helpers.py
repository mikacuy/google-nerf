...
class MotionPotentialMV(nn.Module):
    def __init__(self, input_ch=3, input_ch_feature=384, output_ch=3, skips=[4], no_bias=False):
        ...
        self.motion_mlp = nn.ModuleList(
            ...
            [DenseLayer(512, 512, activation="relu")] + \
            [DenseLayer(512, 512, activation="relu")] + \
        ...
        if not no_bias:
          self.output_index = DenseLayer(128, output_ch, activation="linear")
          self.output_coeff = DenseLayer(128, output_ch, activation="linear")

        else:
          self.output_index = DenseLayer(128, output_ch, activation="linear", bias=False)
          self.output_coeff = DenseLayer(128, output_ch, activation="linear", bias=False)

        self.basis_mlp_ = nn.ModuleList(
            [DenseLayer(output_ch, 256, activation="relu")] + \
            [DenseLayer(256, 256, activation="relu")]
            # [DenseLayer(256, 128, activation="linear")]
            )

        self.output_potential = DenseLayer(256, output_ch, activation="linear", bias=False)
    ...
    def forward(self, x):
        ...
        index = self.output_index(h)
        coeff = self.output_coeff(h)

        potential = index
        for i, l in enumerate(self.basis_mlp_):
            potential = self.basis_mlp_[i](potential)
            potential = F.relu(potential)
        potential = self.output_potential(potential)

        return torch.cat([potential, coeff], dim=-1)
