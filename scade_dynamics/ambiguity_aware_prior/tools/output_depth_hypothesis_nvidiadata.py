...
parser.add_argument('--dump_dir', default= "dump_0718_noalign/", type=str)
...
parser.add_argument('--seed_num', default= 1234, type=int)
...
# DATA_ROOT = os.path.join(FLAGS.dataroot, FLAGS.scene_id, "dense")
DATA_ROOT = os.path.join(FLAGS.dataroot, FLAGS.scene_id)

DUMP_DIR = os.path.join(DATA_ROOT, FLAGS.dump_dir)
...
hypothesis_outdir = os.path.join(DATA_ROOT, "scade_hypothesis/", FLAGS.dump_dir)
if not os.path.exists(os.path.join(DATA_ROOT, "scade_hypothesis")): os.makedirs(os.path.join(DATA_ROOT, "scade_hypothesis"))
...
image_dir = os.path.join(DATA_ROOT, "mv_images")
...
for fol_idx in range(FLAGS.start_frame, len(imgs_list)):
    ...
    print("Curr hypothesis dir")
    print(curr_hypothesis_outdir)
    ...
    with torch.no_grad():
        for i in range(len(imgs_path)):
            ...
            try:
                ...
                print("Error with "+curr_img_path)
                continue
            ...
            cv2.imwrite(os.path.join(curr_temp_fol, img_name+"-raw.png"), rgb_c)
            image_fname.append(os.path.join(curr_temp_fol, img_name+"-raw.png"))
            ...
            for k in range(num_sets):
                ...
                for s in range(mini_batch_size):
                    ...
                    plt.imsave(os.path.join(curr_temp_fol, img_name+'-depth.png'), curr_pred_depth_raw, cmap='rainbow')
            ...
            for k in range(num_sets):
                for s in range(mini_batch_size):
                    ...
                    curr_rbg_name = data['A_paths']

                    fname = curr_rbg_name.split("/")[-1][:-4] + "_" + str(k*mini_batch_size+s) + ".npy"

                    outfname = os.path.join(curr_hypothesis_outdir, fname)
            ...
            output_image_filename = os.path.join(curr_dump_dir, str(i) + '_all.png')
