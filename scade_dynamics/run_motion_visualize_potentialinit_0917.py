...
def viz_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, \
    ...
    ### To save outputs
    result_dir = os.path.join(args.ckpt_dir, args.expname, "visu_potentialinit_sxyz" + str(args.xyz_potential_scale) + "_sdino" + str(args.dino_potential_scale) + "_fw" + str(args.feat_dist_weight) + "_ispca" + str(args.is_dino_pca) + "_pcadim" + str(args.pcadim))
    ...
    with torch.no_grad():
      ...
      for idx in args.camera_indices:
        ...
        XYZ1.append(pnm_points1)
        XYZ2.append(pnm_points2)
        #####################################
      ...
      XYZ1 = torch.cat(XYZ1, 0)
      XYZ2 = torch.cat(XYZ2, 0)

      std_feature1 = torch.std(FEATURES1, dim=0)
      std_feature2 = torch.std(FEATURES1, dim=0)
      std_feature1 = torch.sum(std_feature1)
      std_feature2 = torch.sum(std_feature2)

      std_xyz1 = torch.std(XYZ1, dim=0)
      std_xyz2 = torch.std(XYZ2, dim=0)
      std_xyz1 = torch.sum(std_xyz1)
      std_xyz2 = torch.sum(std_xyz2)
    ...
    with torch.no_grad():
      ...
      # print(potential_norms.shape)

      # max_norm = np.max(potentials1_)
      ...
      # print(potentials_range.shape)
      # print(potentials_range)
      # exit()
      ...
      # potential_color = (potentials1_ - np.min(potentials1_, axis=0))/(np.percentile(potentials1_, 95, axis=0) - np.min(potentials1_, axis=0))


      potential_color = np.clip(potential_color, 0., 1.)
      save_pointcloud_samples(pc1, potential_color, os.path.join(result_dir, "000_database1_potential.png"), save_views=False)
      save_motion_vectors(pc1, color1, potentials1_, os.path.join(result_dir, "000_database1_potential_endpt.png"), is_vec=False)
      ...
      # potential_color = (potentials2_ - np.min(potentials2_, axis=0))/(np.percentile(potentials2_, 95, axis=0) - np.min(potentials2_, axis=0))
      ...
      for idx in args.camera_indices:
        ...
        save_pc_correspondences(pc1, pc2, colors1, colors2, fname + "_nncorres_wholevec.png", save_views=False)
        save_pointcloud_samples(pc2, colors2, fname + "_pc2nn.png", save_views=False)
        save_pointcloud_samples(pc1, colors1, fname + "_pc1.png", save_views=False)
        ...
        potential_color = np.clip(potential_color, 0., 1.)
        save_pointcloud_samples(pc1, potential_color, fname + "_frame1_potential.png", save_views=False)

        save_motion_vectors(pc1, colors1, potentials1_, fname + "_potentials1_endpt.png", is_vec=False)
        ...
        save_pointcloud_samples(pc_frame2, potential_color, fname + "_frame2_potential.png", save_views=False)

        colors_frame2 = pnm_rgb_term2.detach().cpu().numpy()
        save_motion_vectors(pc_frame2, colors_frame2, potentials2_, fname + "_potentials2_endpt.png", is_vec=False)
