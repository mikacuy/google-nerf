...
def render_rays(ray_batch,
    ...
    if not for_motion:
      ...
      ret = {'pnm_rgb_term': pnm_rgb_term, 'pnm_feature_term': pnm_feature_term, 'pnm_points': pnm_pts, "rgb_map":rgb_map}
        ...
        if i % it_recache == 0 or ALL_DATABASE is None:
          ...
          for j in range(2):
            ...
            for idx in range(1, len(i_train), skip_view):
              ...
              with torch.no_grad():
                pnm_rgb_term, pnm_feature_term, pnm_points, extras = get_database(H_database, W_database, intrinsics[j][img_i], chunk=(args.chunk // 4), c2w=poses[j][img_i], **render_kwargs_train)
...
def run_nerf():
    ...
    if args.dataset == "llff":
        ...
        images, depths, valid_depths, poses, H, W, intrinsics, near, far, i_split, \
            video_poses, video_intrinsics, _, features, features_fnames  = load_scene_blender_depth_features(scene_data_dir, scene_feature_dir, downsample=2, feat_dim = args.feat_dim)
