...
def create_nerf2(args, scene_render_params):
    ...
    model1 = NeRF_semantics(D=args.netdepth, W=args.netwidth,
                     ...
                     input_ch_views=input_ch_views, input_ch_cam=args.input_ch_cam, use_viewdirs=args.use_viewdirs, semantic_dim = args.pcadim)
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, \
    ...
    with torch.no_grad():
      ...
      for idx in args.camera_indices:
        ...
        pnm_feature_term1_input = pnm_feature_term1[is_object_ray1].reshape(-1, args.pcadim)
        pnm_feature_term2_input = pnm_feature_term2[is_object_ray2].reshape(-1, args.pcadim)
    ...
    with torch.no_grad():
      ...
      for idx in args.camera_indices:
        ...
        pnm_feature_term1_input = pnm_feature_term1[is_object_ray1].reshape(-1, args.pcadim)  - MEAN_FEATURES1

        ###############################################################
        ### Now we are using purely pca projected vector -- change this
        ###############################################################        
        pnm_feature_term1 = pnm_feature_term1[is_object_ray1].reshape(-1, args.pcadim)
        ...
        pnm_feature_term2_input = pnm_feature_term2[is_object_ray2].reshape(-1, args.pcadim)  - MEAN_FEATURES2

        ###############################################################
        ### Now we are using purely pca projected vector -- change this
        ###############################################################            
        pnm_feature_term2 = pnm_feature_term2[is_object_ray2].reshape(-1, args.pcadim)
      ...
      curr_pnm_rgb_term1, curr_pnm_feature_term1, curr_potentials_term1 = torch.split(DATABASE1, [3, args.pcadim, 3], dim=-1)
      ...
      curr_pnm_rgb_term2, curr_pnm_feature_term2, curr_potentials_term2 = torch.split(DATABASE2, [3, args.pcadim, 3], dim=-1)
    ...
    for i in trange(start, N_iters):
      ...
      pnm_feature_term_query_input = pnm_feature_term_query - MEAN_FEATURES_QUERY
      pnm_feature_term_db_input = pnm_feature_term_db - MEAN_FEATURES_DB
      ...
      curr_entries_query = torch.cat([pnm_rgb_term_query * weight_rgb_query * args.color_dist_weight, pnm_feature_term_query * weight_features_query * args.feat_dist_weight, (potentials_query - selected_query_pts) * weight_potentials_query], -1)          
      curr_entries_db = torch.cat([pnm_rgb_term_db * weight_rgb_db * args.color_dist_weight, pnm_feature_term_db * weight_features_db * args.feat_dist_weight, (potentials_db - selected_database_nn_pts) * weight_potentials_db], -1) 
      ...
      with torch.no_grad():
        ...
        curr_pnm_rgb_term1, curr_pnm_feature_term1, curr_potentials_term1 = torch.split(selected_entries, [3, args.pcadim, 3], dim=-1)
        curr_pnm_rgb_term2, curr_pnm_feature_term2, curr_potentials_term2 = torch.split(selected_database_entries, [3, args.pcadim, 3], dim=-1)
