...
Add DCI-KNN for fast nearest neighbour search
'''
import os
import shutil
import subprocess
...
from model import NeRF_semantics_twobranch, MotionPotential, MotionPotential_nopos, get_embedder, get_rays, sample_pdf, sample_pdf_joint, img2mse, mse2psnr, to8b, \
...
def create_nerf2(args, scene_render_params):
    ...
    ####### First NeRF model ######
    model1 = NeRF_semantics_twobranch(D=args.netdepth, W=args.netwidth,
                    input_ch=input_ch, output_ch=output_ch, skips=skips,
                    input_ch_views=input_ch_views, input_ch_cam=args.input_ch_cam, use_viewdirs=args.use_viewdirs, \
                    semantics_highdim = args.feat_dim, semantics_lowdim = args.pcadim)
    ...
    if args.N_importance > 0:
        model_fine1 = NeRF_semantics_twobranch(D=args.netdepth_fine, W=args.netwidth_fine,
                        input_ch=input_ch, output_ch=output_ch, skips=skips,
                        input_ch_views=input_ch_views, input_ch_cam=args.input_ch_cam, use_viewdirs=args.use_viewdirs, \
                        semantics_highdim = args.feat_dim, semantics_lowdim = args.pcadim)
    ...
    ####### Second NeRF model ######
    model2 = NeRF_semantics_twobranch(D=args.netdepth, W=args.netwidth,
                    input_ch=input_ch, output_ch=output_ch, skips=skips,
                    input_ch_views=input_ch_views, input_ch_cam=args.input_ch_cam, use_viewdirs=args.use_viewdirs, \
                    semantics_highdim = args.feat_dim, semantics_lowdim = args.pcadim)
    ...
    model_fine2 = None
    if args.N_importance > 0:
        model_fine2 = NeRF_semantics_twobranch(D=args.netdepth_fine, W=args.netwidth_fine,
                        input_ch=input_ch, output_ch=output_ch, skips=skips,
                        input_ch_views=input_ch_views, input_ch_cam=args.input_ch_cam, use_viewdirs=args.use_viewdirs, \
                        semantics_highdim = args.feat_dim, semantics_lowdim = args.pcadim)       
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, \
    ...
    fname = os.path.join(args.pretrained_dir, args.ref_expname, "features_pca.joblib")
    pca = joblib.load(fname)
    print("Loaded pca model from reference frame.")
    ...
    with torch.no_grad():
      ...
      for idx in args.camera_indices:
        ...
        pnm_feature_term1_highdim = pnm_feature_term1[..., :args.feat_dim]
        pnm_feature_term1_lowdim = pnm_feature_term1[..., args.feat_dim:]
        pnm_feature_term2_highdim = pnm_feature_term2[..., :args.feat_dim]
        pnm_feature_term2_lowdim = pnm_feature_term2[..., args.feat_dim:]

        ### This is taking the average
        pnm_rgb_term1 = torch.mean(pnm_rgb_term1, axis=-2)
        pnm_rgb_term2 = torch.mean(pnm_rgb_term2, axis=-2)

        pnm_feature_term1_highdim = torch.mean(pnm_feature_term1_highdim, axis=-2)
        pnm_feature_term1_lowdim = torch.mean(pnm_feature_term1_lowdim, axis=-2)
        pnm_feature_term2_highdim = torch.mean(pnm_feature_term2_highdim, axis=-2)
        pnm_feature_term2_lowdim = torch.mean(pnm_feature_term2_lowdim, axis=-2)

        pnm_points1 = torch.mean(pnm_points1, axis=-2)
        pnm_points2 = torch.mean(pnm_points2, axis=-2)

        pnm_feature_term1_input = pnm_feature_term1_lowdim[is_object_ray1].reshape(-1, args.pcadim)
        pnm_feature_term2_input = pnm_feature_term2_lowdim[is_object_ray2].reshape(-1, args.pcadim)
    ...
    with torch.no_grad():
      ...
      for idx in args.camera_indices:
        ...
        pnm_feature_term1_input = pnm_feature_term1_lowdim[is_object_ray1].reshape(-1, args.pcadim)  - MEAN_FEATURES1

        ###############################################################
        ### Now we are using purely pca projected vector -- change this
        ###############################################################        
        pnm_feature_term1_highdim = pnm_feature_term1_highdim[is_object_ray1].reshape(-1,args.feat_dim)
        ...
        pnm_feature_term2_input = pnm_feature_term2_lowdim[is_object_ray2].reshape(-1, args.pcadim)  - MEAN_FEATURES2

        ###############################################################
        ### Now we are using purely pca projected vector -- change this
        ###############################################################            
        pnm_feature_term2_highdim = pnm_feature_term2_highdim[is_object_ray2].reshape(-1, args.feat_dim)
        ...
        curr_entries1 = torch.cat([pnm_rgb_term1, pnm_feature_term1_highdim, potentials1 - pnm_points1], -1)
        curr_entries2 = torch.cat([pnm_rgb_term2, pnm_feature_term2_highdim, potentials2 - pnm_points2], -1)
    ...
    for i in trange(start, N_iters):
      ...
      if i % args.recache_database == 0 or (NN_1_to_2 is None) or (NN_1_to_2 is None):
        print("Caching the database...")

        ### i) Constructing the database
        print("Constructing the database cache...")
        with torch.no_grad():
          CURR_DATABASE1 = []
          CURR_DATABASE2 = []
          ...
          for idx in args.camera_indices:
            ...
            pnm_viewdir1 = extras1_unary["pnm_viewdir"]
            pnm_viewdir2 = extras2_unary["pnm_viewdir"]
            ...
            pnm_rgb_term1 = pnm_rgb_term1[is_object_ray1].reshape(-1, 3)
            pnm_points1 = pnm_points1[is_object_ray1].reshape(-1, 3)
            pnm_viewdir1 = pnm_viewdir1[is_object_ray1].reshape(-1, 3)
            pnm_feature_term1_input = pnm_feature_term1_lowdim[is_object_ray1].reshape(-1, args.pcadim)  - MEAN_FEATURES1
            ...
            curr_entries1 = torch.cat([pnm_rgb_term1 * weight_rgb1 * args.color_dist_weight, pnm_feature_term1_highdim * weight_features1 * args.feat_dist_weight, (potentials1 - pnm_points1) * weight_potentials1, eps1_noise], -1)          

            CURR_DATABASE1.append(curr_entries1)

            DB1_PTS.append(torch.cat([pnm_points1, pnm_viewdir1], -1))

            ### Database 2
            pnm_rgb_term2 = pnm_rgb_term2[is_object_ray2].reshape(-1, 3)
            pnm_points2 = pnm_points2[is_object_ray2].reshape(-1, 3)
            pnm_viewdir2 = pnm_viewdir2[is_object_ray2].reshape(-1, 3)
            pnm_feature_term2_input = pnm_feature_term2_lowdim[is_object_ray2].reshape(-1, args.pcadim)  - MEAN_FEATURES2
            ...
            curr_entries2 = torch.cat([pnm_rgb_term2 * weight_rgb2 * args.color_dist_weight, pnm_feature_term2_highdim * weight_features2 * args.feat_dist_weight, (potentials2 - pnm_points2) * weight_potentials2, eps2_noise], -1)

            CURR_DATABASE2.append(curr_entries2)

            DB2_PTS.append(torch.cat([pnm_points2, pnm_viewdir2], -1))


          CURR_DATABASE1 = torch.cat(CURR_DATABASE1, axis=0)
          CURR_DATABASE2 = torch.cat(CURR_DATABASE2, axis=0)

          DB1_PTS = torch.cat(DB1_PTS, axis=0)
          DB2_PTS = torch.cat(DB2_PTS, axis=0)

          DB1_PTS_CACHE = DB1_PTS
          DB2_PTS_CACHE = DB2_PTS

          print(DB1_PTS_CACHE.shape)
          print(DB2_PTS_CACHE.shape)
          print(CURR_DATABASE1.shape)
          print(CURR_DATABASE2.shape)
          print()
          ###############################################

        ### ii) Get nearest neighbor indices
        with torch.no_grad():
          NN_1_to_2 = torch.empty((CURR_DATABASE1.shape[0]), dtype=torch.int64)
          NN_2_to_1 = torch.empty((CURR_DATABASE2.shape[0]), dtype=torch.int64)

          ### For frame 1 to frame 2
          nn_dataloader1 = torch.utils.data.DataLoader(
              dataset=torch.utils.data.TensorDataset(CURR_DATABASE1),
              batch_size=1024,
              num_workers=0,
              shuffle=False)
          ...
          for j, data in enumerate(nn_dataloader1):
            ...
            distances = torch.norm(selected_entries.unsqueeze(1) - CURR_DATABASE2.unsqueeze(0), p=2, dim=-1) 
          ...
          if args.is_two_way:
            nn_dataloader2 = torch.utils.data.DataLoader(
                dataset=torch.utils.data.TensorDataset(CURR_DATABASE2),
                batch_size=1024,
                num_workers=0,
                shuffle=False)
            ...
            for j, data in enumerate(nn_dataloader2):
              ...
              distances = torch.norm(selected_entries.unsqueeze(1) - CURR_DATABASE1.unsqueeze(0), p=2, dim=-1) 
              _, min_indices = torch.min(distances, axis=-1)
      ...
      if args.is_two_way:
        ...
        if coin_flip % 2 == 0:
          ...
          QUERY = DB1_PTS_CACHE
          DATABASE = DB2_PTS_CACHE

          motion_query_func = render_kwargs_train["motion_network_query_fn"]
          motion_model_q = render_kwargs_train["network_motion1"]
          motion_model_db = render_kwargs_train["network_motion2"]

          network_query_func = render_kwargs_train["network_query_fn"]
          network_q = render_kwargs_train["network_fine1"]
          network_db = render_kwargs_train["network_fine2"]
          ...
          MEAN_FEATURES_QUERY = MEAN_FEATURES1
          MEAN_FEATURES_DB = MEAN_FEATURES2

        else:
          QUERY_NN_IDX = NN_2_to_1
          QUERY = DB2_PTS_CACHE
          DATABASE = DB1_PTS_CACHE
          ...
          network_query_func = render_kwargs_train["network_query_fn"]
          network_q = render_kwargs_train["network_fine2"]
          network_db = render_kwargs_train["network_fine1"]
          ...
          MEAN_FEATURES_QUERY = MEAN_FEATURES2
          MEAN_FEATURES_DB = MEAN_FEATURES1
      ...
      selected_query_entries = torch.gather(QUERY, 0, indices.unsqueeze(-1).repeat(1, 3 + 3)).squeeze()
      selected_database_entries = torch.gather(DATABASE, 0, selected_nn_idx.unsqueeze(-1).repeat(1, 3 + 3)).squeeze()

      selected_query_pts, selected_query_viewdirs = torch.split(selected_query_entries, [3, 3], dim=-1)
      selected_database_nn_pts, selected_database_nn_viewdirs = torch.split(selected_database_entries, [3, 3], dim=-1)

      ### Forward ###
      raw_importance = network_query_func(selected_query_pts.unsqueeze(1), selected_query_viewdirs, render_kwargs_train["embedded_cam"], network_q)
      pnm_rgb_term_query = torch.sigmoid(raw_importance[...,:3]).squeeze()
      pnm_feature_term_query = raw_importance[...,4:].squeeze()

      raw_importance = network_query_func(selected_database_nn_pts.unsqueeze(1), selected_database_nn_viewdirs, render_kwargs_train["embedded_cam"], network_db)
      pnm_rgb_term_db = torch.sigmoid(raw_importance[...,:3]).squeeze()
      pnm_feature_term_db = raw_importance[...,4:].squeeze()

      pnm_feature_term_query_highdim = pnm_feature_term_query[..., :args.feat_dim]
      pnm_feature_term_query_lowdim = pnm_feature_term_query[..., args.feat_dim:]
      pnm_feature_term_db_highdim = pnm_feature_term_db[..., :args.feat_dim]
      pnm_feature_term_db_lowdim = pnm_feature_term_db[..., args.feat_dim:]

      pnm_feature_term_query_input = pnm_feature_term_query_lowdim - MEAN_FEATURES_QUERY
      pnm_feature_term_db_input = pnm_feature_term_db_lowdim - MEAN_FEATURES_DB


      potentials_query =  motion_query_func(selected_query_pts * args.xyz_potential_scale, pnm_feature_term_query_input * args.dino_potential_scale, motion_model_q)      
      potentials_db = motion_query_func(selected_database_nn_pts * args.xyz_potential_scale, pnm_feature_term_db_input * args.dino_potential_scale, motion_model_db)
      ################

      curr_entries_query = torch.cat([pnm_rgb_term_query * weight_rgb_query * args.color_dist_weight, pnm_feature_term_query_highdim * weight_features_query * args.feat_dist_weight, (potentials_query - selected_query_pts) * weight_potentials_query], -1)          
      curr_entries_db = torch.cat([pnm_rgb_term_db * weight_rgb_db * args.color_dist_weight, pnm_feature_term_db_highdim * weight_features_db * args.feat_dist_weight, (potentials_db - selected_database_nn_pts) * weight_potentials_db], -1) 
...
def config_parser():
    ...
    parser.add_argument("--pretrained_fol2", type=str, default="hotdog_singleframe_f2",
                        help='first nerf folder')
    ...
    ### For joint training
    ## pca loading
    parser.add_argument("--ref_expname", type=str, default="hotdog_singleframe_f1",
...
def run_nerf():
    ...
    if args.task == "train":
        ...
        args.expname = args.expname + "_mu" + str(args.pnm_mean) + "_std" + str(args.pnm_std) + "_sxyz" + str(args.xyz_potential_scale) + "_sdino" + str(args.dino_potential_scale) + "_fw" + str(args.feat_dist_weight) + "_pcadim" + str(args.pcadim) + "_mlrate" + str(args.motion_lrate)
