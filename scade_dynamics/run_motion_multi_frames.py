...
from model import NeRF_semantics, MotionPotentialMV, MotionPotential_nopos, get_embedder, get_rays, sample_pdf, sample_pdf_joint, img2mse, mse2psnr, to8b, \
...
def run_motion_potential(inputs, features, fn, embed_fn, bb_center, bb_scale, netchunk=1024*64):
    """Prepares inputs and applies network 'fn'.
    """
    inputs_flat = torch.reshape(inputs, [-1, inputs.shape[-1]])
    inputs_flat = (inputs_flat - bb_center) * bb_scale
    embedded = torch.cat([embed_fn(inputs_flat), features], -1)
...
#     """
#     embed_fn, input_ch = get_embedder(args.multires, args.i_embed, input_dims=4)
#     breakpoint()
#     input_ch_views = 0
#     embeddirs_fn = None
...
#                           use_viewdirs=args.use_viewdirs, semantic_dim = args.feat_dim)
...
def create_nerf2(args, scene_render_params):
    ...
    #####################
    ####### Shared Models with extra time embeding######
    #####################
    network_query_fn = lambda inputs, viewdirs, embedded_cam, network_fn : run_network(inputs, viewdirs, embedded_cam, network_fn,
                                                                embed_fn=embed_fn,
                                                                embeddirs_fn=embeddirs_fn,
    ...
    # breakpoint
    ### Motion model
    # Zhengqi: I add xyzt with positional encoding
    if not args.is_dino_pca:
      if not args.potential_nopos:
        motion_model1 = MotionPotentialMV(input_ch=input_ch, input_ch_feature=args.feat_dim, output_ch=3 * args.L)
      else:
        motion_model1 = MotionPotential_nopos(input_ch=input_ch, input_ch_feature=args.feat_dim, output_ch=3 * args.L)
    else:
      if not args.potential_nopos:
        motion_model1 = MotionPotentialMV(input_ch=input_ch, input_ch_feature=args.pcadim, output_ch=3 * args.L)
      else:
        motion_model1 = MotionPotential_nopos(input_ch=input_ch, input_ch_feature=args.pcadim, output_ch=3 * args.L)
...
def render_rays(ray_batch,
    ...
    # time embedding
    img_idx_rep = torch.ones_like(pts[..., 0:1]) * float(idx) / video_length * 2. - 1.
    pts_t = torch.cat([pts, img_idx_rep], -1)

    # if idx == 0:
    raw = network_query_fn(pts_t, viewdirs, embedded_cam, network_fn1)
    # else:
    ...
    if N_importance == 0:
        ...
        raw = network_query_fn(pts_t, viewdirs, embedded_cam, run_fn)
    ...
    if for_motion:
      ...
      # time embedding
      img_idx_rep = torch.ones_like(pnm_pts[..., 0:1]) * float(idx) / video_length * 2. - 1.
      pnm_pts_t = torch.cat([pnm_pts, img_idx_rep], -1)

      raw_importance = network_query_fn(pnm_pts_t, viewdirs, embedded_cam, run_fn)
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, \
    ...
    test_images = images # T x N x H x W x 3
    test_poses = poses # T x N x 4 x 4
    test_intrinsics = intrinsics # T x N x 4
    i_test = i_test
    ...
    with torch.no_grad():
        ...
        for idx in args.camera_indices:
            img_i = i_train[idx]      
            # loop each time step
            for tt in range(args.video_length):
                print(img_i, tt)
                #### Downsample to get a smaller size
                curr_valid_depth1 = valid_depths[tt][img_i]
                ...
                _, _, _, extras1_unary = render(H_database, W_database, intrinsics[tt][img_i], chunk=(args.chunk // 8), 
                    c2w=poses[0][img_i][:3,:4], with_5_9=False, idx=tt, video_length=args.video_length,
                    for_motion=True, **render_kwargs_train)
                ...
                FEATURES[tt].append(pnm_feature_term1_input)

        MEAN_FEATURES = {}
        for tt in range(args.video_length):
            FEATURES[tt] = torch.cat(FEATURES[tt], 0) # Nx3
            MEAN_FEATURES[tt] = torch.mean(FEATURES[tt], dim=0) # 3
        # FEATURES2 = torch.cat(FEATURES2, 0)
        # MEAN_FEATURES2 = torch.mean(FEATURES2, dim=0) # 3

    ### Save feature mean ###
    for tt in range(args.video_length):
        output_dict = {"frame%d"%tt: MEAN_FEATURES[tt].detach().cpu().numpy()}
    ...
    with torch.no_grad():
        ...
        for idx in args.camera_indices:
            ...
            for tt in range(args.video_length):
                ...
                _, _, _, extras1_unary = render(H_database, W_database, intrinsics[tt][img_i], 
                    chunk=(args.chunk // 8), c2w=poses[tt][img_i][:3,:4], 
                    with_5_9=False, idx=tt, video_length=args.video_length, 
                    for_motion=True, **render_kwargs_train)
                ...
                if args.is_dino_pca:
                    ...
                    # pred_features_img = (pred_features - comp_min) / (comp_max - comp_min)
                    pnm_feature_term1_input = torch.from_numpy(pred_features_img).to(pnm_points1.device)
                    pnm_feature_term1_input = pnm_feature_term1_input[is_object_ray1].reshape(-1, args.pcadim)  - MEAN_FEATURES[tt]
                else:
                    pnm_feature_term1_input = pnm_feature_term1[is_object_ray1].reshape(-1, args.feat_dim)  - MEAN_FEATURES[tt]

                pnm_feature_term1 = pnm_feature_term1[is_object_ray1].reshape(-1, args.feat_dim)

                img_idx_rep = torch.ones_like(pnm_points1[..., 0:1]) * float(tt) / args.video_length * 2. - 1.
                pnm_points1_t = torch.cat([pnm_points1, img_idx_rep], -1)

                output_motions = motion_query_func(pnm_points1_t * args.xyz_potential_scale, 
                    pnm_feature_term1_input * args.dino_potential_scale, 
                    motion_model1)

                # position is weighted sum of potential and coeff
                num_coeff_dim = 3 * args.L
                potentials1 =  output_motions[..., 0:num_coeff_dim].reshape(-1, args.L, 3)
                coeff1 = output_motions[..., num_coeff_dim:].reshape(-1, args.L, 3)
                h_1 = torch.sum(potentials1 * coeff1, dim=1)

                curr_entries1 = torch.cat([pnm_rgb_term1, pnm_feature_term1, h_1 - pnm_points1], -1)

                DATABASES[tt].append(curr_entries1)
            # END OF TIME FOR LOOP
        # END OF CAMERA FOR LOOP

        for tt in range(args.video_length):
            DATABASES[tt] = torch.cat(DATABASES[tt], 0)
            curr_pnm_rgb_term1, curr_pnm_feature_term1, curr_potentials_term1 = torch.split(DATABASES[tt], [3, args.feat_dim,3], dim=-1)
            ...
            SCALE_FACTORS.append(np.array([weight_rgb1.detach().cpu().numpy(), 
                weight_features1.detach().cpu().numpy(), 
                weight_potentials1.detach().cpu().numpy()]))

        print("Computed scales for energies:")
        print(SCALE_FACTORS) # 2 of 3

    #### Save scale factors ###
    for tt in range(args.video_length):
        output_dict = {"frame%d"%tt: SCALE_FACTORS[tt]}
    ...
    print("========== Precompute samples, color and feature =============")
    with torch.no_grad():
        #  List of dictionary
        DATABASE_PRECOMPUTE = [{} for _ in range(args.video_length)]
        ...
        for idx in args.camera_indices:
            ...
            for tt in range(args.video_length):
                ...
                DATABASE_PRECOMPUTE[tt][img_i] = curr_entries1
    ###############################################

    ##########################################
    ##### Training Motion Potential Network ####
    ##########################################
    print("############# Training Motion Potential Network #############")
    for i in trange(start, N_iters):
        DATABASES = [[] for _ in range(args.video_length)]

        if args.visu:
            POTENTIAL_ONLY = [[] for _ in range(args.video_length)]
            PTS_COLOR_ONLY = [[] for _ in range(args.video_length)]
            NOISE_VECTOR = [[] for _ in range(args.video_length)]
        ...
        for idx in args.camera_indices:
            ...
            for tt in range(args.video_length):
                print(img_i, tt)
                pnm_points1, pnm_rgb_term1, pnm_feature_term1, pnm_feature_term1_input = torch.split(DATABASE_PRECOMPUTE[tt][img_i], [3, 3, args.feat_dim, args.pcadim], dim=-1)
                ...
                #### Construct database elements ####
                weight_rgb1, weight_features1, weight_potentials1 = SCALE_FACTORS[tt]
                curr_entries1 = torch.cat([pnm_rgb_term1 * weight_rgb1 * args.color_dist_weight, 
                    pnm_feature_term1 * weight_features1 * args.feat_dist_weight, 
                    (h_1 - pnm_points1) * weight_potentials1, eps1_noise], -1)

                DATABASES[tt].append(curr_entries1)
                if args.visu:
                    with torch.no_grad():
                        POTENTIAL_ONLY[tt].append((h_1))
                        PTS_COLOR_ONLY[tt].append(torch.cat([pnm_points1, pnm_rgb_term1], dim=-1))
                        NOISE_VECTOR[tt].append(eps1_noise)
        ...
        for tt in range(args.video_length): 
            ...
            if args.visu:
                POTENTIAL_ONLY[tt] = torch.cat(POTENTIAL_ONLY[tt], 0)
                PTS_COLOR_ONLY[tt] = torch.cat(PTS_COLOR_ONLY[tt], 0)
                NOISE_VECTOR[tt] = torch.cat(NOISE_VECTOR[tt], 0)
        # breakpoint()
        ...
        # randomly choose start frame index
        idx_1 = np.random.randint(0, args.video_length - 1)
        idx_2 = idx_1 + 1
        ...
        if args.is_two_way:
            ...
            if coin_flip % 2 == 0:
                QUERY = DATABASES[idx_1]
                DATABASE = DATABASES[idx_2]

                if args.visu:
                    DATABASE_PTS_COLOR = PTS_COLOR_ONLY[idx_2]
                    DATABASE_NOISE = NOISE_VECTOR[idx_2]
                    QUERY_PTS_COLOR = PTS_COLOR_ONLY[idx_1]

            else:
                QUERY = DATABASES[idx_2]
                DATABASE = DATABASES[idx_1]

                if args.visu:
                    DATABASE_PTS_COLOR = PTS_COLOR_ONLY[idx_1]
                    DATABASE_NOISE = NOISE_VECTOR[idx_1]
                    QUERY_PTS_COLOR = PTS_COLOR_ONLY[idx_2]
        ...
        with torch.no_grad():
            ...
            curr_pnm_rgb_term1, curr_pnm_feature_term1, curr_potentials_term1, _ = torch.split(selected_entries, 
                [3, args.feat_dim, 3, 1], dim=-1)
            curr_pnm_rgb_term2, curr_pnm_feature_term2, curr_potentials_term2, _ = torch.split(selected_database_entries, 
                [3, args.feat_dim, 3, 1], dim=-1)
        ...
        if i % 200 == 2 and args.visu:
            ...
            potentials1_ = POTENTIAL_ONLY[idx_1].detach().cpu().numpy()
            ...
            save_pointcloud_samples(pc1, potential_color, fname + "_potential_%03d.png"%idx_1, pc_size=0.05, skip=1)

            ### potential visualization
            potentials2_ = POTENTIAL_ONLY[idx_2].detach().cpu().numpy()
            ...
            save_pointcloud_samples(pc2, potential_color, fname + "_potential_%03d.png"%idx_2, pc_size=0.05, skip=1)

            ###########################
            ## Flow for the whole PC ##
            ###########################
            db1_to_db2_nn_idx = knn_points(DATABASES[idx_1].unsqueeze(0), DATABASES[idx_2].unsqueeze(0), K=1).idx
            db1_to_db2_nn_idx = db1_to_db2_nn_idx[0]

            db2_to_db1_nn_idx = knn_points(DATABASES[idx_2].unsqueeze(0), DATABASES[idx_1].unsqueeze(0), K=1).idx
            db2_to_db1_nn_idx = db2_to_db1_nn_idx[0]

            selected_database_entry_potential1 = torch.gather(POTENTIAL_ONLY[idx_2], 0, db1_to_db2_nn_idx.repeat(1,3)).squeeze()
            potential2_nn = selected_database_entry_potential1

            selected_database_entry_potential2 = torch.gather(POTENTIAL_ONLY[idx_1], 0, db2_to_db1_nn_idx.repeat(1,3)).squeeze()
            potential1_nn = selected_database_entry_potential2   

            pc1 = PTS_COLOR_ONLY[idx_1][:, :3].detach().cpu().numpy()
            color1 = PTS_COLOR_ONLY[idx_1][:, 3:].detach().cpu().numpy()

            pc2 = PTS_COLOR_ONLY[idx_2][:, :3].detach().cpu().numpy()
            color2 = PTS_COLOR_ONLY[idx_2][:, 3:].detach().cpu().numpy()

            #### Visualize flow per iteration
            scene_flow = POTENTIAL_ONLY[idx_1] - potential2_nn
            pc1_flowed = pc1 - scene_flow.detach().cpu().numpy()
            save_pointcloud_samples(pc1_flowed, color1, fname + "_database_pcflowed_%03dto%03d.png"%(idx_1, idx_2), pc_size=0.05, skip=1)
            save_point_cloud(pc1_flowed, to8b(color1), fname + "_database_pcflowed_%03dto%03d.ply"%(idx_1, idx_2))

            scene_flow = POTENTIAL_ONLY[idx_2] - potential1_nn
            pc2_flowed = pc2 - scene_flow.detach().cpu().numpy()
            save_pointcloud_samples(pc2_flowed, color2, fname + "_database_pcflowed_%03dto%03d.png"%(idx_2, idx_1), pc_size=0.05, skip=1)
            save_point_cloud(pc1_flowed, to8b(color1), fname + "_database_pcflowed_%03dto%03d.ply"%(idx_2, idx_1))
        ################## END OF VIZ 
...
def config_parser():
    ...
    parser.add_argument("--pretrained_dir", type=str, default="/home/zhengqili/filestore/Mika/google-nerf/scade_dynamics/data_multiframe_hotdog",
...
def run_nerf():
    ...
    if args.dataset == "llff":
        ...
        ### Frame 1 ###
        # Load multi-time data
        scene_id_list = []
        video_length = args.video_length
        for t in range(video_length):
            if t == 0:
                scene_id_list.append(("hotdog_single_plate_detached", "hotdog_single_plate_detached_dino_features_small"))
            else:
                scene_id_list.append(("hotdog_single_plate_detached_f%d"%t, "hotdog_single_plate_detached_f%d_dino_features_small"%t))

        for scene_id, feature_dir in scene_id_list:
            scene_data_dir = os.path.join(args.data_dir, scene_id)        
            scene_feature_dir = os.path.join(args.data_dir, feature_dir)
            print("scene_data_dir ", scene_data_dir)
            print("scene_feature_dir ", scene_feature_dir)

            images, depths, valid_depths, poses, H, W, intrinsics, near, far, i_split, \
                video_poses, video_intrinsics, _, features, features_fnames  = load_scene_blender_depth_features(scene_data_dir, 
                    scene_feature_dir, 
    ...
    print("all_valid_depths", all_valid_depths.shape)
    ...
    # add time dimension
    args.bb_center = torch.cat([(max_xyz + min_xyz) / 2., torch.ones_like(min_xyz[0:1])], dim=0)
    bb_scale = torch.ones_like(args.bb_center) * 2. / (max_xyz - min_xyz).max()
    bb_scale[-1] = 1.
    args.bb_scale =  bb_scale
    # breakpoint()
