...
def create_nerf(args, scene_render_params):
    ...
    for i in range(len(args.frame_idx)):
    ...
    optimizer = torch.optim.Adam(params=params_to_optimize, lr=args.lrate, betas=(0.9, 0.999))
    start = 0
    ##########################
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, scales_init=None, shifts_init=None, use_depth=False):
    ...
    # create nerf model
    render_kwargs_train, render_kwargs_test, start, optimizer = create_nerf(args, scene_sample_params)
...
def config_parser():
    ...
    parser.add_argument("--pretrained_dir", type=str, default="/home/mikacuy/Desktop/coord-mvs/google-nerf/scade_dynamics/log_walking_sparse_cameras_half/",
...
def run_nerf():
    ...
    if args.task == "train":
        train_nerf(all_images, None, None, all_poses, all_intrinsics, i_split, args, scene_sample_params, lpips_alex, None, None, all_depth_hypothesis, use_depth=True)
        exit()
 
    # create nerf model for testing
    _, render_kwargs_test, _, _ = create_nerf(args, scene_sample_params)
