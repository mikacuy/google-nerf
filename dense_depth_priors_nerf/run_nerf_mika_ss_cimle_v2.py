...
1018: Combine opt_ss with cIMLE for white balancing experiments --> coarse space carving make it joint too.
'''
import os
import shutil
import subprocess
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis):
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_print==0:
            ...
            scale_mean = torch.mean(DEPTH_SCALES)
            shift_mean = torch.mean(DEPTH_SHIFTS)
