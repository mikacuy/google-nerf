...
sys.path.append(os.path.join(BASE_DIR, "ResizeRight/"))
import resize_right
import interp_methods
print("Imported resize right.")
...
def get_ray_batch_from_one_image_hypothesis_idx_ss(H, W, img_i, images, depths, valid_depths, poses, intrinsics, all_hypothesis, curr_scale_resized, curr_shift_resized, args):
    coords = torch.stack(torch.meshgrid(torch.linspace(0, H-1, H), torch.linspace(0, W-1, W), indexing='ij'), -1)  # (H, W, 2)
    # img_i = np.random.choice(i_train)
    ...
    curr_scale_selected = curr_scale_resized[select_coords[:, 0], select_coords[:, 1]]
    curr_shift_selected = curr_shift_resized[select_coords[:, 0], select_coords[:, 1]]
    ...
    if args.depth_loss_weight > 0.:
        ...
        batch_rays = torch.stack([rays_o, rays_d], 0)  # (2, N_rand, 3)
    return batch_rays, target_s, target_d, target_vd, img_i, target_h, curr_scale_selected, curr_shift_selected, space_carving_mask
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, is_init_scales=False, scales_init=None, shifts_init=None):
    ...
    if not is_init_scales:
        DEPTH_SCALES = torch.autograd.Variable(torch.ones((images.shape[0], args.lowres_size, args.lowres_size, 1), dtype=torch.float, device=images.device)*args.scale_init, requires_grad=True)
        DEPTH_SHIFTS = torch.autograd.Variable(torch.ones((images.shape[0], args.lowres_size, args.lowres_size, 1), dtype=torch.float, device=images.device)*args.shift_init, requires_grad=True)
    else:
        print(scales_init.shape)
        print(shifts_init.shape)
        DEPTH_SCALES = torch.autograd.Variable(torch.from_numpy(scales_init).to(device).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,args.lowres_size,args.lowres_size,1), requires_grad=True)
        DEPTH_SHIFTS = torch.autograd.Variable(torch.from_numpy(shifts_init).to(device).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,args.lowres_size,args.lowres_size,1), requires_grad=True)        
    ...
    for i in trange(start, N_iters):
        ...
        # print(curr_scale)
        # print(curr_shift)
        # print(curr_scale.shape)
        # print(curr_shift.shape)

        curr_scale_resized = resize_right.resize(curr_scale, scale_factors=None, out_shape=(H, W, 1),
                    interp_method=interp_methods.linear, support_sz=None,
                    antialiasing=False, by_convs=False, scale_tolerance=None,
                    max_numerator=10, pad_mode='replicate')
        curr_shift_resized = resize_right.resize(curr_shift, scale_factors=None, out_shape=(H, W, 1),
                    interp_method=interp_methods.linear, support_sz=None,
                    antialiasing=False, by_convs=False, scale_tolerance=None,
                    max_numerator=10, pad_mode='replicate')

        # print(curr_scale_resized.shape)
        # print(curr_shift_resized.shape)
        # print(images[img_i].shape)
        # print()

        ## Scale and shift
        batch_rays, target_s, target_d, target_vd, img_i, target_h, curr_scale_selected, curr_shift_selected, space_carving_mask = get_ray_batch_from_one_image_hypothesis_idx_ss(H, W, img_i, images, depths, valid_depths, poses, \
            intrinsics, all_depth_hypothesis, curr_scale_resized, curr_shift_resized, args)

        # print(batch_rays.shape)
        # print(target_h.shape)
        # print(curr_scale_selected.shape)
        # print(curr_shift_selected.shape)
        # print(curr_scale_selected)
        # print(curr_shift_selected)
        # print(space_carving_mask.shape)
        # exit()

        target_h = target_h*curr_scale_selected + curr_shift_selected        
        ...
        if args.space_carving_weight>0. and i>args.warm_start_nerf:
            space_carving_loss = compute_space_carving_loss(extras["pred_hyp"], target_h, is_joint=args.is_joint, mask=space_carving_mask)
...
def config_parser():
    ...
    parser.add_argument('--refresh_z', default= 100000, type=int, help='Number of iterations to recache latent code')
    ...
    ### Load scaleshift inits
    parser.add_argument('--init_scales', default= False, type=bool) 
    ...
    ### Size of low-res scale/shift image
    parser.add_argument('--lowres_size', type=int, default=8, help='N = size of low res image to upsample. (NxN)')
...
def run_nerf():
    ...
    if not args.init_scales:
        ...
        scales_init, shifts_init = load_scene_mika(scene_data_dir, args.cimle_dir, args.num_hypothesis, 'transforms_train.json', args.init_scales, args.scales_dir, args.use_gt_init)
    ...
    if args.task == "train":
        if not args.init_scales:
            ...
            train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_depth_hypothesis, args.init_scales, scales_init, shifts_init)
