...
def load_scene_leres(basedir, cimle_dir, num_hypothesis=20, train_json = "transforms_train.json"):
    splits = ['train', 'val', 'test', 'video']
    ...
    ############################################    
    #### Load to jointly train leres+cIMLE ####
    ############################################    
    ## For now only for train poses
    train_idx = i_split[0]
    ...
    for i in range(len(train_idx)):
        filename = filenames[train_idx[i]]
        leres_img = read_leres_image(basedir, filename)
        leres_depth = read_leres_depth(basedir, filename, depth_scaling_factor, near, far)

        all_leres_imgs.append(leres_img)
        all_leres_depths.append(leres_depth)

    all_leres_imgs = torch.stack(all_leres_imgs)
    all_leres_depths = torch.stack(all_leres_depths)
    #########################################


    return imgs, depths, valid_depths, poses, H, W, intrinsics, near, far, i_split, gt_depths, gt_valid_depths, all_leres_imgs, all_leres_depths
