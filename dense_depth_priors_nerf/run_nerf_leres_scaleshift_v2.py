...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths, all_leres_imgs, all_leres_depths):
    ...
    #### Initialize scale and shift for each view --> removes least squares wrt to gt ####

    # DEPTH_SHIFTS = torch.autograd.Variable(torch.randn((all_leres_imgs.shape[0], 1), dtype=torch.float, device=all_leres_imgs.device), requires_grad=True)
    # DEPTH_SCALES = torch.autograd.Variable(torch.randn((all_leres_imgs.shape[0], 1), dtype=torch.float, device=all_leres_imgs.device), requires_grad=True)

    DEPTH_SCALES = torch.autograd.Variable(torch.ones((all_leres_imgs.shape[0], 1), dtype=torch.float, device=all_leres_imgs.device)*0.05, requires_grad=True)
    DEPTH_SHIFTS = torch.autograd.Variable(torch.ones((all_leres_imgs.shape[0], 1), dtype=torch.float, device=all_leres_imgs.device)*0.0, requires_grad=True)
    ...
    #         # leres_depth = all_leres_depths[idx]
    ...
    #         # X = torch.linalg.lstsq(pred_depth_temp, leres_depth).solution.squeeze()

    #         # # print(pred_depth_temp.squeeze())
    #         # # print()
    #         # # print(leres_depth.squeeze())
    #         # # print()
    #         # # print(X)
    #         # # print(X.shape)

    #         # scale = torch.mean(X[:, 0], axis=0)
    #         # shift = torch.mean(X[:, 1], axis=0)
    ...
    #         DEPTH_SCALES[idx] = scale
    #         DEPTH_SHIFTS[idx] = shift
    ...
    print(DEPTH_SHIFTS)
    print()
    optimizer.add_param_group({"params": DEPTH_SCALES, "lr": args.scaleshift_lr})
    optimizer.add_param_group({"params": DEPTH_SHIFTS, "lr": args.scaleshift_lr})
    ...
    ### z-codes
    z_all = torch.normal(0.0, 1.0, size=(images.shape[0], args.num_hypothesis, args.leres_dlatent))

    for i in trange(start, N_iters):

        ### Resample z every n number of iterations
        if ((i-1) % args.i_weights) == 0:
            print("Resampling z codes")
            z_all = torch.normal(0.0, 1.0, size=(images.shape[0], args.num_hypothesis, args.leres_dlatent))
        ...
        # curr_scale = torch.gather(DEPTH_SCALES, 0, torch.from_numpy(np.array([img_i])).unsqueeze(-1).to(leres_img.device)).squeeze()
        # curr_shift = torch.gather(DEPTH_SHIFTS, 0, torch.from_numpy(np.array([img_i])).unsqueeze(-1).to(leres_img.device)).squeeze()

        curr_scale = DEPTH_SCALES[img_i]
        curr_shift = DEPTH_SHIFTS[img_i]

        ## Scale and shift
        all_hypothesis = pred_depth*curr_scale + curr_shift
        ...
        nn.utils.clip_grad_value_(DEPTH_SCALES, 0.1)        
        nn.utils.clip_grad_value_(DEPTH_SHIFTS, 0.1) 
        ...
        if i%args.i_weights==0:
            ...
            ## Save LeReS model
            save_name = os.path.join(args.ckpt_dir, args.expname, 'leres_{:06d}.pth'.format(i))
            torch.save({
                'model_state_dict': leres_model.state_dict(),
                'optimizer': leres_optimizer.optimizer.state_dict()},
                save_name, pickle_module=dill)
            print("Saved leres checkpoint.")
