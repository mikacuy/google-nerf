...
# parser.add_argument('--output_image_height', type=int, default=720, help='export image height')
...
def main():
    ...
    sens_file = os.path.join(opt.scannet_path, scene_name, scene_name + '.sens')
    label_path = os.path.join(opt.scannet_path, scene_name, opt.label_type)
    ...
    output_color_path = os.path.join(opt.output_path, scene_name, 'rgb')
    
    if os.path.exists(os.path.join(opt.output_path, scene_name)) and os.path.exists(output_color_path + '/0.jpg') \
        and os.path.exists(os.path.join(opt.output_path, scene_name, 'label')) and os.path.exists(os.path.join(opt.output_path, scene_name, 'label', '0.png')):
        print(scene_name + ' already extracted!')
        exit()

    if opt.export_label_images:
        output_label_path = os.path.join(opt.output_path, scene_name, 'label')
    ...
    output_depth_path = os.path.join(opt.output_path, scene_name, 'depth')
    if not os.path.isdir(output_depth_path):
        os.makedirs(output_depth_path)
    output_pose_path = os.path.join(opt.output_path, scene_name, 'pose')
    ...
    sd.export_intrinsics(os.path.join(opt.output_path, scene_name))
